...
**The corpus is not reducible.** A Phase 0 recommendation proposed moving AMD-003, AMD-004, and AMD-005 to on-demand reading as "Phase-8 physiology." Architecture Rev 1.1 settles that: **AMD-004** defines `Signal`, a `brokkr-core` type built in **Phase 1**; **AMD-005** governs EIR, without which the architecture contained a prohibited one-way ratchet; **AMD-003** governs KVASIR, now in scope by DAP decision. All fourteen imports are load-bearing. None is deferred.
