...
**Date:** 6 September 2026 (Rev 1.19)
...
| 4 | **Enhanced Conformance (OQGF-E)** | **NOT CLAIMED — declared target, eight requirements open** | §1.4 declares the target; §14 records the verdicts. Open (`PARTIAL`) at this revision: **OQGF-M-1** (attestation `measurements` unverified, no issuer), **OQGF-M-6** (`reconciliation_pass_rate` is a declared placeholder — `Score(0)` in every committed fixture; nothing computes it), **OQGF-A-3** (RFC 3161 timestamp authority is a seam, absence recorded), **OQGF-R-6** (threshold key custody; framework tier ambiguity referred upward), **OQGF-P-11.2** (minimization declared, not verified), **OQGF-P-12.3** (envelope declared, environment not attested), **OQGF-P-12.6** (sub-agent type surface, no runtime path), **OQGF-P-12.8** (recording satisfied; evidence-capture independence open — F-23). Each is a named residual in §13. **A declared target is not a conformance claim, and a partial verdict is not a rounding error.** |
...
| 6 | **High-Assurance Readiness (OQGF-H)** | **NOT CLAIMED** | §1.4 declares BROKKR *architected toward* High-Assurance, which forecloses nothing and claims nothing. OQGF-R-5 (cross-jurisdictional audit replication) is a dated deferral; OQGF-R-6 threshold custody, second-DAP review, continuous environment attestation at `lifetime/4`, and dual-family Canary/probe obligations are High-Assurance criteria BROKKR does not meet today. |
...
— End of BROKKR technical architecture, Rev 1.19.
