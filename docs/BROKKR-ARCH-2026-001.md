...
**Date:** 22 September 2026 (Rev 1.32)
...
**Disposes:** Rev 1.32 **disposes GAP-2026-09-18-001** by removing `VendorTrustScore::signature` — a field **nothing computed, nothing verified, and for which no signed content was ever defined.** It removes a field and **supplies no property**, because the property a reader would infer from it is already supplied one level up: `write_vendor_trust_score` is called from `write_model_endpoint`, which is called from `endpoints_signed_content`, so the entire score — all six factors, the Rev 1.31 `evidence` pair, `reviewed`, and `reviewer` — is inside the bytes the endpoints register signature covers, and **promotion-gate predicate 2 verifies that signature against the DAP root.** The score was tamper-evident and DAP-attributable before this revision and remains so after it. What the field added was **the shape of an independent attestation with no mechanism behind it** — **the OQGF-R-6.2 defect inverted**: there, a custody posture asserted for eighteen revisions with no field behind it; here, a field with no claim behind it, and the second is the easier to miss because **a field *looks* like implementation.** §6.2's type snippet is corrected, and a second staleness found at Step 0 is corrected with it — the snippet had never shown the `evidence` field Rev 1.31 placed, so **it misdescribed the type in two directions at once**, over-stating what was attested and under-stating what was recorded. **Rev 1.32 also declines a rule on the record:** the consequence-claim rule recommended at Rev 1.30 §6.5 and carried in GAP-2026-09-18-001 §7 is **declined** (§15), with a pointer placed at §6.5 so a reader of the recommendation sees its disposition. Two one-line corrections ride with it: **§14's OQGF-M-6 row cited the wrong crate** for `VendorTrustScore` (`brokkr-genome`; the type is declared in `brokkr-core::genome`), and §6.2's snippet is brought into agreement with the type it depicts. **No verdict moves.** OQGF-M-6 stays `PARTIAL` for Rev 1.31's reason — the fifth factor is unmeasured and unmeasurable from the current stream — and **nothing beneath that row moved**: the five corpus factors, the type-level presence guarantee, predicate 4's staleness arithmetic, and the endpoints-register attribution all stand exactly as they did. **I-10 is unaffected**: it requires a signed *Endpoint Registry*, which is verified.
...
| 2 | **Software State** | **13 crates; 451 tests green; HEAD `cc13097`** | `brokkr-{adapt,audit,barrier,bifrost,cli,core,crypto,gate,genome,intent,reasoner,sentinel,tools}` in the workspace. **451 passed / 0 failed / 19 ignored, measured at `cc13097` on a quiet tree** (`cargo test --workspace --locked`, sole occupant, read only after confirming both `cargo` and `rustc` had exited — a completion notification firing on a wrapper while cargo still ran produced a false count once, and the check is now part of the procedure; the 19 ignored are the live suites, of which the 17 live OQGF-A-3 tests were run separately with `--ignored` and pass; the five new OQGF-P-11.2 tests are hermetic and run in this count). The prior figure, "Workspace GREEN 409/0" from commit `a53cafb`, is superseded. §14 traces the requirement each crate carries. |
...
**What the eight open items are actually waiting on — checked item by item, because the convenient summary of this list is wrong.** It is tempting to say that nothing here waits on code in this repository, since most of it is deployment or assessment. **Seven of eight, yes. One, no.**

| Open item | Waiting on | Code here? |
|---|---|---|
| **OQGF-R-6.2** `ABSENT` | A hardware key store and a genuinely two-party issuance procedure. Element 3, the CBOM declaration, **is built** — `Cbom::custody`, `Genome::tier`, and promotion-gate predicate 7 (`brokkr-genome/src/gate.rs:46`) | **No** — deployment and organization |
| **OQGF-P-12.3** | OS-level confinement: seccomp-bpf, a network namespace, a read-only root, dropped `capabilities(7)`. The probe **is built** (`brokkr-cli/src/probe.rs`) | **No** — deployment |
| **OQGF-A-3** clause 2 | An operating RFC 3161 responder, and one that signs post-quantum for the clause to be *met* rather than merely wired. The client is built and tested | **No, in the sense that matters** — the one-line wiring at `brokkr-cli/src/main.rs:363` has nothing to point at until a responder exists |
| **OQGF-M-1** | An attestation issuer, and a source of expected measurements to verify against | **No** — neither exists to consume |
| **OQGF-P-12.6** | Sub-agents to exist. BROKKR spawns none; the enforcement hook is placed and untested against real spawning | **No** — a capability decision, not a backlog item |
| **OQGF-P-12.8** | An observation path external to and unmodifiable by the governed agent. The recording half is complete; the sensor is the orchestrator and says so (F-23) | **No** — an independent sensor is by definition not inside the system it observes |
| **OQGF-P-11.2** | An assessor judging whether a signed field list is minimal | **No** — not a predicate |
| **OQGF-M-6** | **Code in this repository.** `reconciliation_pass_rate` is a `Score` with **no producer** — grep finds it declared (`genome.rs:307`), encoded (`brokkr-genome/src/canonical.rs:390`), and set to `Score(0)` in every fixture. **Nothing computes it**, and no accumulator over reconciliation outcomes exists in `brokkr-sentinel`. Runtime data would feed it; something must still count | **Yes** |

**The exception is recorded rather than rounded away.** "Nothing is waiting on code" is the kind of summary that is 87% true, reads as clean, and is the exact shape of claim this revision exists to correct. M-6 needs an accumulator, and a reader planning work should know that one of the eight is a build task and seven are not.
...
pub struct VendorTrustScore {
    ...
    /// Evidence behind the MEASURED factor above (Rev 1.31). `observations: 0`
    /// means that factor is UNMEASURED rather than measured at zero — the
    /// distinction promotion-gate predicate 8 enforces.
    pub evidence: FactorEvidence,
    pub reviewed: Timestamp,
    pub reviewer: Dap,
    // NO `signature` (removed Rev 1.32). Attribution and tamper-evidence come from
    // the endpoints register that carries this score, whose signature predicate 2
    // verifies. A field here would be a second attestation with no verifier.
}
```
...
#### The score's attestation comes from the register that carries it

*(Rev 1.32; disposes GAP-2026-09-18-001.)* Every revision from Rev 1.2 to Rev 1.31 carried a `signature: DualSignature` on `VendorTrustScore`. **Nothing computed it, nothing verified it, and no signed content was ever defined for it to cover.** The field is removed, and this subsection records what that removal does, what it deliberately does not decide, and what it costs.

**What was verified at HEAD before removing it**, rather than inherited from the gap report:

| # | Check | Result |
|---|---|---|
| 1 | A signed-content function for `VendorTrustScore` | **None.** `brokkr-genome/src/canonical.rs` exports exactly seven: `tools_`, `cbom_`, `aibom_`, `endpoints_`, `roots_`, `policy_`, and `genome_signed_content`. **There was nothing for the field to be a signature *over*.** |
| 2 | Any verification site | **None.** A workspace grep for `trust_score` conjoined with `sig` returns no hit at all — not a verifier, not a signer, only the encoder that writes the bytes as data. |
| 3 | What predicate 2 verifies | **Six register signatures plus the genome's** — `Register::{Tools, Cbom, Aibom, Endpoints, Policy, Roots}` and `GenomeSignatureInvalid`. The trust score's was not among them. |
| 4 | What fixtures put in it | **A placeholder.** `dummy_sig()` in `brokkr-genome/tests/promotion.rs` (four zero bytes per family) and `dual_sig()` in `brokkr-core/tests/negative_invariants.rs`. |

**The property a reader would infer from the field is real, and a different mechanism supplies it.** `write_vendor_trust_score` is called from `write_model_endpoint`, which is called from `write_endpoints_signed` and therefore `endpoints_signed_content`. **The entire score — all six factors, the Rev 1.31 `evidence` pair, `reviewed`, and `reviewer` — is inside the bytes the endpoints register signature covers**, and promotion-gate predicate 2 verifies that signature against the DAP root. Two consequences follow, and they are why the removal loses nothing:
...
**So the field supplied no property not already supplied one level up.** It was not an unprotected value. It was **a redundant field shaped like an independent attestation**, where the attestation is actually supplied by the register that carries the score.

**Why that is a defect and not a harmless spare field.** A signature field is a claim that something was attested. A reader of the type — or an assessor reading this section's snippet, which is how an assessor reads a type — concludes the vendor trust score carries its own attestation and is independently verifiable. **It did not and it was not.** The code comment at the encoder was honest about this and said so plainly; **the code was not lying. The type misled, and so did this section**, whose snippet showed the field with no qualification.

**This is the OQGF-R-6.2 shape, inverted.** There, the architecture asserted a custody posture for eighteen revisions with **no field** behind it — a claim with nothing to check (§1.4, §6.11). Here there was **a field with no claim behind it** — the shape of an assurance, with no mechanism and no content. Both mislead a reader in the same direction, and **the second is the easier to miss, because a field *looks* like implementation.**

##### What the removal is not

**It is not a judgment that a separate signer is unwanted.** A deployment in which vendor risk is signed by a party *other than* the endpoints-register signer is a real and defensible model: a procurement or supplier-assessment function separate from whoever configures model endpoints, which is entirely plausible for a federal buyer, and which would make a second attestation under a second key meaningful rather than duplicative.

**That model does not exist today, and a field with no verification site is not a hook for it.** An unverified field does not reserve a place for a future control; it asserts a present one. If the separate-signer model arises, it is **placed then — as a design, with a signed-content function, a signing party named, and a promotion-gate predicate that verifies it** — because a second attestation is worth only what its verification site is worth. Adding the mechanism to an existing field would be the same work as adding it to a new one, so nothing is foreclosed by removing the field and nothing was reserved by keeping it.

##### The cost, verified rather than repeated

Removing the field changes `write_vendor_trust_score`, which moves `endpoints_signed_content` and therefore `genome_signed_content`. **That is the third consecutive revision to move those bytes** — Rev 1.29's `PersonalDataTag`, Rev 1.31's `FactorEvidence`, and this.

**Three claims bound the cost at zero today. Each was checked at HEAD rather than carried forward, because the Rev 1.29 cost paragraph was wrong for exactly this reason** (GAP-2026-09-10-001: a consequence-claim that was checkable, unchecked, and would have been relied on):

| Claim | Verified |
|---|---|
| SAGA is in memory | `SagaState` holds `records: Vec<AuditRecord>`, initialised `Vec::new()`. No store, no file, no load path. |
| Nothing is persisted | **No workspace crate declares a `serde` dependency at all.** Nothing in the tree serialises a genome or a register, so no stored digest can move. |
| Fixtures sign live | The promotion fixture computes `kp.sign_dual(&canonical::endpoints_signed_content(&endpoints))` at test time, so an encoding change **moves with** the fixture rather than breaking it. |

**The audit chain does not move, and this was traced rather than assumed.** `AuditEvent::GenomePromotion` carries `version` and the **declared** `corpus_digest`, and `write_genome_promotion` encodes exactly those two — neither derived from any register encoding. The stronger check is structural: `brokkr-audit` depends only on `brokkr-core` and `brokkr-crypto` (I-5), **so `VendorTrustScore` is not reachable from `record_signed_content` by any path**, not merely absent from the one path inspected. That is the distinction the Rev 1.29 error turned on — *is this type inside an audit record?* rather than *is this type an audit record?* — and it is answered here by dependency direction, which cannot be answered wrongly by looking at one encoder and missing a second.

**Zero now is not zero later.** Once a genome is persisted or a chain is stored, an encoding change carries a migration cost, and the three checks above are the ones that will stop being true first.

##### Two corrections to this section's own text

**The snippet showed a field that no mechanism backed, and omitted a field that one does.** Correcting the first surfaced the second at Step 0: the snippet had never been updated to show `evidence: FactorEvidence`, which Rev 1.31 placed. **It therefore misdescribed the type in two directions at once** — over-stating what was attested and under-stating what was recorded — in the one artifact an assessor reads to evaluate OQGF-M-6. Both are corrected above.

**The dated Rev 1.31 §5.4 table below is not rewritten.** Its predicate-8 row enumerates the struct's fields as they stood, `signature` among them, under a heading that states it was *"verified against the committed tree at `53f2c33`."* **A dated record of what was checked when is history, not current text** (§8), and rewriting it would destroy the record of what the field list was at the moment predicate 8 was placed. A reader comparing that row against the snippet above will find they disagree, which is correct: they describe different revisions and each says which.

##### Step 0 — every site that read, wrote, encoded, or documented the field

**§5.4 places no obligation here, and saying so is the point of this table.** §5.4 governs *"any predicate, gate, or rule placed against a value"*; **Rev 1.32 places no rule** — it removes a field — so there is no evaluator whose path to a value must be named. What replaces that table is the enumeration a removal actually needs: every site that touches the field, so that none is missed and none is discovered later.

| Site | Kind | Disposition |
|---|---|---|
| `brokkr-core/src/genome.rs` — `VendorTrustScore::signature` | the declaration | removed |
| `brokkr-genome/src/canonical.rs` — `write_vendor_trust_score` | the encoder, and its two-line comment naming the field | the `write_dual_signature` call and the comment are removed; `write_dual_signature` itself stays, used by every register encoder |
| `brokkr-genome/tests/promotion.rs` — the `endpoint(..)` fixture | a construction site | the initialiser is removed; `dummy_sig()` stays — it has ten other uses as a pre-signing placeholder, and its doc comment, which names the trust score as the case where a signature is data the gate does not check, is corrected |
| `brokkr-core/tests/negative_invariants.rs` — `trust_score()` | a construction site | the initialiser is removed; `dual_sig()` stays, with eight other uses |
| §6.2's type snippet | documentation | corrected above, together with the missing `evidence` field |
| §6.2's Rev 1.31 §5.4 predicate-8 row | a **dated** verification record | **not rewritten** (§8) |
| **Nothing else** | — | No verifier, no signer, no other crate. `VendorTrustScore` is named in exactly five files workspace-wide: its declaration, the `brokkr-core` re-export, the genome encoder, and the two fixtures. |

##### What this does not change

**No verdict moves, and no verdict ever rested on the field.** OQGF-M-6's text requires five factors and a quarterly review; **it imposes no attestation obligation on the score itself.** The obligations it does impose are checked by mechanisms that work and are untouched: predicate 4 for staleness, a type-level guarantee for factor presence (no factor is an `Option`), and the endpoints register signature for tamper-evidence and attribution. **OQGF-M-6 stays `PARTIAL` for Rev 1.31's reason** — the fifth factor is unmeasured, and measuring it from the current stream would fabricate it.

**Nothing beneath that verdict moved either.** The five corpus factors, the retained `data_handling` tightening, the `evidence` pair, predicate 4's arithmetic, and predicate 8's honest-state exemption all stand exactly as Rev 1.31 left them. What changed is that the type no longer implies a second attestation it never had. **I-10 is unaffected:** it requires a signed *Endpoint Registry*, which is exactly the signature that carries the score.

#### The fifth factor cannot be measured yet, and computing it anyway would fabricate it

*(Rev 1.31; disposes GAP-2026-09-15-001.)* Rev 1.4 placed `reconciliation_pass_rate` and recorded it *"awaiting measurement."* Rev 1.11 corrected the phase at which it would arrive. **Neither checked whether the measurement would mean anything, and it would not.**

**The input is degenerate.** `reconciliation_deviation` (`brokkr-sentinel/src/heimdall.rs`) reports a deviation when an `Observation::Hop` carries `executed: Some(e)` with `e != authorized`. The orchestrator's only successful-execution emission is `brokkr-cli/src/lib.rs:897`:
...
**`executed` is a clone of `authorized`** — asserted, not derived. `Tool::execute` returns `ToolOutcome { output: String }` (`brokkr-tools/src/tool.rs:20`), documented *"opaque to the governance spine — a tool's output is data, not authority"*, and nothing compares it to anything. **So no production emission can produce a deviation.** Every executed hop is a pass by construction, and a counter over that input returns **exactly 1.0, always.**

**Why no counter is placed, when the counter is the easy part.** `HeimdallState` already holds `governed: u64` and `incidents: Vec<HostHarmIncident>` with `roll_window()` closing a window; a pass/fail pair modelled on them is a small, testable change. **The reason to refuse it is what the number would assert.**

| | What it is |
|---|---|
| **`Score(0)` today** | **Inert.** Nothing computes the field and nothing reads it as a measurement. It asserts nothing; it merely sits at the bottom of the range |
| **A computed `Score(100)`** | **An affirmative false claim** — signed into the genome, dated, DAP-attributed, read by the OQGF-G-4 gate, and **indistinguishable at assessment from a genuine measurement** |

An auditor requesting evidence for the fifth factor would be shown a number produced by comparing a value to a copy of itself. **That is the R-6.2 defect class** — AMD-018 §AMD.2.1: *a declared model that overstates the separation actually achieved is a conformance failure, not a documentation defect* — and it is §7's prohibition on fabricated completion, relocated from a report into a signed register, where it is harder to find and carries more authority.

**The reading of the factor is not reopened.** OQGF-M-6's phrase is *"statistical reconciliation pass rate"* — OQGF-M-3's vocabulary, and M-3 is `n.a.` for BROKKR (§1.4, no quantum workload). Rev 1.4 read it as OQGF-M-12 cross-hop behavioural reconciliation, and that reading stands. **Taking the M-3 reading would make the factor `n.a.` and this problem disappear, which is reading a requirement down until it vanishes** — the move this architecture has refused since Rev 1.2, and it would be especially convenient here.

#### A score cannot currently say "no evidence", and `window_rate` already knows better

`Score` is `pub struct Score(pub u8)` — *"a 0..=100 trust/score value (OQGF-M-6)"*, higher is better. **It has no value meaning "unmeasured."** `Score(0)` is the worst possible score, not a neutral one, so the type forces a factor with no evidence to claim either total failure or some invented middle.

**The correct instinct is already committed, one crate away.** `window_rate` (`brokkr-sentinel/src/heimdall.rs`) returns `f64::NAN` rather than `0.0` for an empty window, with the reasoning in its own doc comment: *"a window that evaluated no governed actions has no evidence, and a rate of `0.0` would claim safety from none."* **The host-harm rate can say "no evidence." A trust score cannot.** That asymmetry is the defect.

Two shapes were considered against what is committed, and the second is placed.

**Refused — `Option<Score>` on each factor.** It expresses absence exactly, and it is wrong here for a reason the type hides. It makes *absence* representable but says nothing about **how much evidence** a present score rests on: `Some(Score(100))` from one observation and from ten thousand are indistinguishable, and the first is the more dangerous. It would also change six fields of a signed structure to serve one.

**Placed — an evidence count beside the measured factor.**

```rust
/// Evidence behind a measured factor of a `VendorTrustScore` (OQGF-M-6).
///
/// `Score` is a `u8` and cannot say "no evidence"; this says it. **Zero observations is the
/// honest state of `reconciliation_pass_rate` today**, and it is distinguishable from a
/// measured zero — which `Score(0)` alone is not. The same distinction `window_rate` already
/// draws by returning `NaN` rather than `0.0` for an empty window, carried into a register
/// where the value is signed rather than computed on demand.
pub struct FactorEvidence {
    /// Reconciliation outcomes the score was computed over. **Zero means unmeasured**, and
    /// the promotion gate refuses a genome whose measured factor claims evidence it has none of.
    pub observations: u64,
    /// When the measurement window closed. Distinct from `VendorTrustScore::reviewed`, which
    /// is when a **DAP reviewed the score** — a review can restate an old measurement, and
    /// conflating the two would let a fresh review launder stale evidence.
    pub measured: Timestamp,
}
```

**What the promotion gate does with it — an eighth predicate, stated as the seven are (§6.2):**

| # | Predicate | Fails when |
|---|---|---|
| 8 | **A measured factor is supported by evidence** | `reconciliation_pass_rate` is not `Score(0)` while `evidence.observations == 0` — a factor claiming a measurement it has no observations behind |

**The direction is deliberate and it is the only direction that is safe.** The predicate does **not** refuse a genome whose score is unmeasured — BROKKR's own genome is in exactly that state and must remain promotable. It refuses a genome that **claims a measurement without evidence**, which is the failure this whole subsection is about. A gate refusing the honest state and admitting the dishonest one would be precisely backwards.

**`Score(0)` with `observations == 0` therefore becomes the honest encoding of "unmeasured"**, and the pair — not either half — is what carries the meaning.

#### The fixtures are inert today and become a false claim the moment the field is computable

Both committed fixtures carry `reconciliation_pass_rate: Score(0)` — `brokkr-genome/tests/promotion.rs:94` and `brokkr-core/tests/negative_invariants.rs:520`.

**Today that is inert, and the distinction matters.** Nothing computes the field and nothing reads it as a measurement, so `Score(0)` asserts nothing; it is a placeholder occupying the bottom of a range. **It is not currently a false claim.**

**It becomes one the moment a producer exists.** A fixture carrying a computed-looking `Score(0)` beside a field that *is* computable asserts a 0% reconciliation pass rate for that supplier — the worst measurement obtainable — from no observations at all. **What a fixture should carry once the field is computable is `Score(0)` paired with `FactorEvidence { observations: 0, .. }`**: the same byte, now meaning "unmeasured" rather than "measured at zero", and refusable by predicate 8 if a future edit raises the score without raising the evidence.

**No fixture is changed by this revision**, because nothing is implemented. The point of recording it is that the fixtures are one producer away from asserting something false, and nobody editing that producer would think to look at them.

#### What closing the fifth factor requires — scope, not design

**The executor must report what it did, distinguishably from what it was told to do.**

`ToolOutcome`'s opacity is **not** the defect. It is the correct posture for tool output — §1.2's principle that a proposal carries no authority extends to a result, and a spine that parsed tool output for meaning would be taking instruction from the thing it governs. **It is nonetheless exactly why no reconcilable value exists**: an opaque `String` cannot be compared to an `Action`.

What is missing is an **effect description**: what the execution actually did, expressed in the governance vocabulary the authorization was expressed in, so the two are comparable without interpreting prose.

| Crate | What it would carry |
|---|---|
| `brokkr-core` | The effect type the spine compares against `Action` — governance vocabulary, not tool-specific detail |
| `brokkr-tools` | Every `Tool::execute` produces one. **This is where the cost lives**, and where the hard question is |
| `brokkr-cli` | Derive `executed` from that value instead of cloning `authorized` |

**Deliberately not designed here.** The hard question is not the plumbing but what an effect description *is* for a tool whose effect is a file write, versus a shell command, versus a network call — and whether a tool that cannot honestly describe its own effect should be **refusable at the genome** rather than silently unreconcilable. That is a placement, and the builder does not place.

#### §5.4 — what Rev 1.31 places, and how each value was verified

*(Rev 1.31.)* One type and one predicate are placed; everything else in this subsection is a correction or a scope note. Verified against the committed tree at `53f2c33`.

| Placed rule | Value it reads | Path — named accessor, named crate | How verified |
|---|---|---|---|
| Predicate 8 | the measured factor | `VendorTrustScore::reconciliation_pass_rate` — `brokkr-core` | **observed**: declared at `genome.rs:307`, already encoded by `write_score` at `brokkr-genome/src/canonical.rs:390`, and already reached by the gate's staleness predicate (4) |
| Predicate 8 | the evidence count | `FactorEvidence::observations` — `brokkr-core` | **placed here.** No such field exists; `VendorTrustScore` carries six `Score`s, `reviewed`, `reviewer`, `signature` |
| Predicate 8 | the genome under promotion | `promote(genome, dap_key, now)` — `brokkr-genome` | **observed**: predicates 1–7 already read the genome from this parameter |
| The honest-state exemption | that an unmeasured factor stays promotable | the same two values | **observed by construction**: the predicate fires on *claimed measurement without evidence*, so `Score(0)` + `observations: 0` passes |
| **Not placed** | a reconciliation pass/fail counter | `Heimdall::observe` holds the `Observation` and already classifies it | **Reachable, and deliberately not placed.** The input is degenerate: `lib.rs:897` clones `authorized` into `executed`, so every outcome is a pass and the counter would fabricate 1.0 |
| **Not placed** | an effect description to compare against `Action` | — | **No path.** `ToolOutcome` carries an opaque `String` (`brokkr-tools/src/tool.rs:20`) and nothing else describes what an execution did. Scoped above; a placement, not a builder's invention |

**The fifth row is the one that matters.** It is the first entry in any §5.4 table in this document to record a value as **reachable and still not placed** — every prior "not placed" row was blocked by an absent path. Reachability was the right question and is not the only one: *can the evaluator obtain this value* was answered yes, and *does the value mean anything* was answered no.
...
#### Minimization (OQGF-P-11.2) — declared field scope, deterministically enforced

The requirement, quoted rather than paraphrased:

> **OQGF-P-11.2 (Minimization).** A conforming system SHALL collect and retain Personal Data only to the extent necessary for a declared Purpose. Personal Data admitted to a Privileged Context — a training corpus, evaluation dataset, fine-tuning corpus, model registry, or any AIBOM-governed artifact (OQGF-I-11, OQGF-G-2) — SHALL be minimized to what the declared Purpose requires. Bulk admission of Personal Data beyond the declared Purpose, or "collect everything in case it is useful," SHALL NOT satisfy this requirement.

Rev 1.7 placed the ingress rule that a Personal datum entering a Privileged Context must carry a declared Purpose, and recorded the rest as a residual: *"the declaration is gated, the minimization is not."* This subsection places the missing half — **and Step 0 changed its shape, which is recorded here rather than smoothed over.**

##### What Step 0 found, and why the check reads declarations rather than content

The natural reading of "minimized to what the Purpose requires" is a rule that inspects admitted content and refuses fields the Purpose does not need. **That rule is unbuildable against the committed tree, and it would be the wrong rule if it were buildable.**

**Unbuildable.** `Barrier::evaluate(&self, flow: &BoundaryFlow, now: Timestamp)` has exactly two parameters. `BoundaryFlow::Ingress` carries `datum: DatumRef`, `personal: Option<PersonalDataTag>`, `bcr: Option<BoundaryCustodyRecord>`, and `context: ContextClass`. **`DatumRef` is a `string_id!` newtype** (`brokkr-core/src/ids.rs:41`), declared in the same block as `SubjectId` and `ToolId` and documented as *"a reference to a datum crossing a boundary"* — an opaque identifier. **There is no content in `BoundaryFlow`, no bytes, and no accessor that would yield any.** A field-level rule over admitted content has no path to a value, which is §5.4's stopping condition.

**Wrong even if buildable, and this is the more important half.** The barrier's egress and ingress decisions are a **Deterministic Gate** under OQGF-P-2. AMD-007 designates content inference — judging what unlabeled data *is* — a **Heuristic Response**, and §6.5 already states the consequence: the content sentinel is *"a backstop to, never a replacement for, the deterministic enforcement of declared classification."* **A Deterministic Gate SHALL NOT take its input from a heuristic one.** Giving the barrier content to inspect would not merely add a capability; it would put a probabilistic judgment inside a fail-closed gate and collapse the innate/adaptive boundary AMD-002 exists to hold.

**So the check compares two declarations, which is exactly what BROKKR already does for classification.** Nothing reads a payload to decide its sensitivity: a classification is *declared*, the gate enforces the declaration deterministically, and a heuristic sentinel flags suspected mislabelling without deciding anything. **Minimization is placed the same way rather than as a second pattern.**

**Three surfaces are required and none exists.** `Purpose` is `{ description: String }` (`brokkr-core/src/personal_data.rs`) — opaque prose that cannot express a field list. Nothing on the flow or the BCR declares which fields a datum carries. And `BarrierCondition`'s eight committed variants contain nothing that says *a field outside the declared purpose scope*.

##### The declaration — where the allowed field set lives, and who signs it

**Minimization is a judgment made in advance by an accountable person, not a computation the gate performs.** Whether a birth date is *necessary* for a fraud-scoring purpose is a governance question about the purpose, answerable only with knowledge of why the data is collected. No predicate over the data decides it, and a design implying the gate determines necessity would be claiming a capability no gate has. **What the gate can do is hold someone to a declaration they signed.**

**The allowed field set belongs in the genome's `PolicyRegister`, not on `Purpose`.** `PolicyRegister` (§6.2) is the signed, versioned, DAP-owned home of exactly this kind of statement — the capability vocabulary, the invariant predicates, the disallowed algorithms — and a purpose→field-set map is the same species of governance declaration.

```rust
/// The field set a declared Purpose is permitted to admit (OQGF-P-11.2).
///
/// A DAP judgment made in advance. Minimization is not decidable from the data: whether a
/// field is *necessary* for a purpose is answerable only from why the data is collected.
/// The gate does not determine necessity — it holds a declaration to what was signed.
pub struct PurposeFieldPolicy {
    /// Matched against the `Purpose` declared on a crossing.
    pub purpose: Purpose,
    /// The closed set of fields this purpose may admit. A closed vocabulary for the same
    /// reason `policy.capabilities` is one (Rev 1.5): without it a typo in a declared field
    /// name is undetectable and silently permanent.
    pub allowed: Vec<FieldName>,
}
```

**Placing it on `Purpose` instead was considered and refused, and the reason is the whole security argument.** A `Purpose` travels *on the crossing*, inside the BCR. If the allowed set travelled with it, the party assembling the data would declare both the fields it is admitting **and** the fields it is permitted to admit — and the check would compare a producer's claim against the same producer's claim. **That is the circular shape §7 names**, and it is the identical defect Rev 1.22 refused when it declined to let `promote` take a caller-supplied tier: *the thing being governed does not supply the terms of its own governance.* The allowed set must be reachable only from a signed artifact the producer cannot edit.

**The barrier reaches it through a resolver seam, because `brokkr-barrier` SHALL NOT depend on `brokkr-genome` (I-5).** The pattern is already in the crate: `Huth<C: EndpointCeiling, A: AcceptanceResolver>` is generic over injected resolver traits precisely so the barrier does not know where a policy came from. A third joins them:

```rust
/// Resolve a declared Purpose to the field set the signed policy permits it (OQGF-P-11.2).
///
/// `None` means the purpose is not declared in the policy — which is a REFUSAL, not a
...
**`None` refusing rather than permitting is load-bearing.** The alternative — an unresolvable purpose admitting everything — would make the control removable by declaring a purpose nobody registered. This is OQGF-P-2's fail-closed posture applied to a lookup, and the same direction §6.2 chose when an invariant with no declared predicate denies rather than passes.

##### The present field set — declared on the crossing, covered by the BCR signature

The other half of the comparison is what the crossing claims to carry. It rides on `PersonalDataTag`, which already carries the purpose and retention and already appears **both** on the flow and inside the BCR:
...
pub struct PersonalDataTag {
    ...
    /// Inside the BCR's signed content, so it is attributable and tamper-evident.
    pub fields: Vec<FieldName>,
}
```

**It is signed, and that is not incidental.** `bcr_signed_content` calls `write_personal`, which encodes the purpose and retention (`brokkr-barrier/src/canonical.rs:90`). Extending it to encode `fields` puts the declaration inside the dual-family signature, so a producer cannot alter what a datum claims to carry after the fact without breaking the record.

**The cost is real, and Rev 1.29 stated it wrongly. The correction is the substance of this revision (GAP-2026-09-10-001).**

Rev 1.29 wrote, as the basis for bounding the cost:

> ~~*"A canonical-encoding change is exactly the class of change Rev 1.10 was written about, and the check that it does not disturb the audit chain is separate: a BCR is not an `AuditRecord`, `record_signed_content` does not encode it, and no `prev` digest covers it."*~~ **SUPERSEDED by Rev 1.30.** The first clause is true; the second and third are false.

**`PersonalDataTag` reaches `record_signed_content` by two paths**, both in `brokkr-audit/src/canonical.rs`, and neither is the barrier's encoder:

```
record_signed_content            :718   the bytes every `prev` digests and every GenerationSignature signs
  └─ write_event
     ├─ AuditEvent::BarrierCrossing   :648
     │    └─ write_flow               :382
     │         └─ write_personal      :394 (Egress) / :406 (Ingress)
     │              └─ write_personal_tag
     └─ RecordedInput::Shredded       :524
          └─ write_personal_tag       :531
```

**`brokkr-audit` keeps its own copy of the personal-tag encoder**, distinct from `brokkr-barrier`'s `write_personal`. Rev 1.29 named the barrier's copy and did not look for a second one. So the change moves the chain-linkage digest of any record carrying a `BarrierCrossing` event or a `Shredded` recorded input — precisely the input Rev 1.10 established as the linkage basis.

**Why the sentence read as correct, which is the part worth keeping.** It answers a real question, accurately: *is a BCR an `AuditRecord`?* No — and that is what it checks. The question that determines the cost is *is a BCR **inside** one?* — and it is, because **OQGF-I-13 requires it to be**: *"Every Barrier decision — a crossing allowed, denied, or quarantined, in either direction — SHALL be recorded in Organ 5."* `AuditEvent::BarrierCrossing` carries a whole `BoundaryFlow`, personal tag included. **The architecture's own requirement is what puts the tag in the chain, and the paragraph denying it stood two sections from the requirement mandating it.**

**The corrected cost.** Changing the encoders changes `bcr_signed_content` **and** `record_signed_content`. **Zero cost today** — SAGA holds `records: Vec<AuditRecord>` in memory, `brokkr-audit` has no `serde` dependency and serialises nothing across runs, so no stored digest moves; the four committed fixtures that construct the affected shapes move *with* the encoding rather than break (`brokkr-barrier/tests/barrier.rs:58`, `brokkr-bifrost/tests/bifrost.rs:129`, `brokkr-audit/tests/audit.rs:68`, and `brokkr-genome/tests/promotion.rs:192` for `PolicyRegister`). **Not zero once SAGA persists**, and that is the whole point of correcting the record: the sentence would have been read by whoever assesses the next encoding change, under exactly the condition that makes it expensive.

**The property is delivered more strongly than Rev 1.29 claimed.** Its stated aim was that the field declaration be signed and tamper-evident. It is covered by the BCR's dual-family signature **and** by the audit chain's hash linkage and generation signatures — **two independent attestations, not one.** The revision undersold its own mechanism while misdescribing its cost, and both halves of that are worth recording: an architecture that overstates is the familiar failure, and one that understates its guarantee while misstating its blast radius is the same carelessness pointed the other way.

**No existing rule would have caught this, and none is invented for it.** §5.4 governs whether a placed *rule* can reach its inputs — here every value is reachable and the design builds. §5.3 governs conformance verdicts. §7 binds the *builder's* summaries, and this claim was made by the DAP in a placed revision. Rev 1.10's discipline is the principle at stake and exists as a constraint on the encoding, not as a check applied to revisions that touch types feeding it. **The gap was found the way the previous three were — a builder grounding a placed design before writing code — and that practice is already mandatory.** Adding a rule per incident is how a framework acquires ceremony rather than control, so none is added here. *(A recommendation, labelled per §4 because it would cost the builder work on every future revision rather than save any: if the DAP wants a rule, the narrow one worth considering is that **a revision asserting a consequence about committed code names the file and symbol it checked**, which is what the §5.4 table already does for inputs. Recommended, not placed.)*

**Disposition, recorded here so §6.5 does not read as an open question.** That recommendation was **declined at Rev 1.32**, on the record and with reasons, in §15. Its text is preserved verbatim above and in GAP-2026-09-18-001 §7, which carries the argument for and against at equal length. **The sentence above is left exactly as Rev 1.30 placed it** (§8); this line states its disposition, it does not amend it.

##### The deterministic check

At ingress into a **Privileged Context**, where a Personal datum is admitted, in addition to the provenance and declared-Purpose conditions Rev 1.7 placed:

| # | Condition | On failure |
|---|---|---|
| 10 | The declared `purpose` resolves in the signed policy | `Deny` — an undeclared purpose has no allowed set |
| 11 | Every field in `tag.fields` appears in the resolved `allowed` set | `Deny` — the datum carries a field outside its purpose's scope |

**The check reads four values and none of them is content:** the declared purpose, the declared present-field set, the resolved allowed-field set, and the context class. Set containment over two declarations.

**It refuses rather than quarantines, and that differs from the surrounding ingress rules deliberately.** Quarantine (OQGF-I-11) is the right verdict for *unprovenanced* data, because provenance can be established afterwards and the datum is then admissible unchanged. A datum carrying fields outside its purpose's scope is not waiting on a missing fact — **it is the wrong data for this purpose**, and the fix is to minimize it before admission or to change the signed policy. Holding it would imply a later step could make it admissible as it stands.

**The AMD-006 path past it is unchanged**: a deliberate, bounded admission is an Accountable Risk Acceptance, scoped, expiring, DAP-signed, with the finding kept visible — never suppression.

##### The failure variant

`BarrierCondition`'s committed variants are `MissingCustodyRecord`, `DatumMismatch`, `SignatureInvalid`, `Expired`, `ClassificationMismatch`, `UnauthorizedDestination`, `ChannelStrengthCollapse`, and `PersonalDataUndeclared`. **None says a field is outside the declared purpose scope**, and the nearest, `PersonalDataUndeclared`, means the *opposite* thing — that no purpose was declared at all. Reporting an over-broad admission as an undeclared one would send an operator to add a declaration that is already present.

```rust
    /// The datum declares a field the signed policy does not permit its declared Purpose
    /// (OQGF-P-11.2). Distinct from `PersonalDataUndeclared`, which means no Purpose was
    /// declared: here the Purpose IS declared, and the data exceeds it.
    PersonalDataFieldOutOfScope,
```

**This is the seventh time this repository has found an error type unable to state what happened** — after `ToleranceError::SignatureInvalid` (Rev 1.12), `ResolveError::{Expired, ReplayedNonce}` (Rev 1.13), `AdaptError::{SubstitutedCorpus, StaleCorpus}`, `TimestampError`'s two splits (Rev 1.24, Rev 1.26), and the three-way EKU split (Rev 1.27). The recurrence has one cause, named at Rev 1.27 and unchanged here: **at the moment of writing, the author knows which condition failed, and the reader will not.** A finding flows into a `BarrierFinding`, which is what an operator reads and what an AMD-006 acceptance is scoped to by `finding_id()` — so a vague condition is not only an unclear message, it is **an acceptance scoped to the wrong thing**.

##### The heuristic backstop, and what it is not

The declared-field check enforces that a datum matches **what it says it carries**. It cannot detect a datum that *lies* — a declaration listing three fields over content carrying five.

**That is the content sentinel's job (OQGF-I-12), and it does not gate.** It may compare declared fields against what the content appears to contain and raise a finding through HEIMDALL's evaluation loop (§6.7), where it is subject to central-tolerance screening and suppressible by a signed Tolerance Grant like any other heuristic detection. **It never turns a declared-conformant crossing into a refusal.**

**A heuristic that gated would violate OQGF-P-2 directly**, and the violation would be structural rather than a matter of degree: tolerance applies to the trained layer and never to a Deterministic Gate, so a gating content inspector would be a fail-closed control that a Tolerance Grant could silence — the exact hole `ToleranceController::grant` returns `NonSuppressibleGate` to prevent. It would also make the barrier's verdict non-deterministic, so two runs over identical declarations could disagree.

**What the backstop can detect:** a declaration that visibly understates structured content it can parse. **What it cannot:** semantic over-collection inside declared fields — a `notes` field declared and permitted, carrying a medical history. That is the covert-channel residual (§13) in a new place, and no field-level control reaches it.

##### What this proves, and what it does not

**It proves that admitted content matches a signed declaration, and that the declaration is within a field set a DAP signed in advance.** Both halves are attributable, tamper-evident, and gate-enforced.

**It does not prove the declaration is minimal.** A DAP who signs a `PurposeFieldPolicy` listing every field the organization holds gets that enforced faithfully — the gate would be working exactly as designed while OQGF-P-11.2's substance went unmet. **This is the same residual shape as the self-declared Capability Envelope (P-12.3) and the self-declared custody model (R-6.2)**, and it is recorded in those terms deliberately: AMD-018 §AMD.2.1's sentence governs here too — *a declared model that overstates what is actually achieved is a conformance failure, not a documentation defect* — and testing it is an assessor's work against a live deployment, not a predicate.

**What the design changes is what an over-broad policy costs.** Today minimization is unstated: nothing declares a field scope, so nothing can be inconsistent with one. After it, an over-broad scope is a **signed, dated, DAP-attributed artifact in the genome**, reviewable in a diff and enumerable at assessment. That is the same argument §6.11 makes for the custody declaration, and it carries the same limit: **the declaration is not the control, and a field containing a claim is not evidence for the claim.**

##### What is built, clause by clause

OQGF-P-11.2 has two halves and only one is code. Recording them separately is the same discipline Rev 1.28 applied to OQGF-A-3, and for the same reason: a single verdict over a requirement with parts conceals which part is met.

| Half | State | What it rests on |
|---|---|---|
| **The declaration is enforced** | **Built** | `FieldName` (`brokkr-core/src/ids.rs:50`, a `string_id!` newtype — `Eq`/`Ord`/`Hash`, the vehicle `Capability` already uses); `PurposeFieldPolicy` and `PolicyRegister::purpose_fields` (`genome.rs:421`, `:457`); `PersonalDataTag::fields` (`barrier.rs:231`); `BarrierCondition::PersonalDataFieldOutOfScope` (`barrier.rs:45`); `PurposeFieldResolver` (`brokkr-barrier/src/resolver.rs:115`); conditions 10 and 11 in `Huth::ingress` |
| **The declaration is minimal** | **Not code, and not closable by code** | An assessment finding against a live deployment — below |

**Tests, named per verdict**, in `brokkr-barrier/tests/barrier.rs`:

| Verdict | Test |
|---|---|
| An in-scope subset is admitted — the policy need not be exhausted, only not exceeded | `test_oqgf_p_11_2_in_scope_fields_are_admitted` |
| An out-of-scope field is refused, with `PersonalDataFieldOutOfScope` and **not** `PersonalDataUndeclared` | `test_oqgf_p_11_2_out_of_scope_field_is_refused` |
| An unresolvable purpose refuses | `test_oqgf_p_11_2_unresolvable_purpose_refuses` |
| An empty allowed set admits a datum declaring no fields and refuses any declared field | `test_oqgf_p_11_2_empty_allowed_set_admits_nothing_but_permits_an_empty_datum` |
| A non-privileged context is not gated | `test_oqgf_p_11_2_non_privileged_context_is_not_gated` |

**An unresolvable purpose refuses rather than permits, and that direction is load-bearing.** `PurposeFieldResolver::allowed_fields` returning `None` denies. The alternative — an unregistered purpose admitting everything — **would make the control removable by declaring a purpose nobody registered**, which is not a weaker control but an absent one. This is OQGF-P-2's fail-closed posture applied to a lookup, and the direction §6.2 already chose when an invariant with no declared predicate denies rather than passes.

**The last test is not padding.** OQGF-P-11.2 gates admission into a Privileged Context; gating a non-privileged one would be *stricter than the requirement*, and under OQGF-P-1 an over-tight gate is a governance failure of equal standing to a missed threat. A control that refuses more than it was asked to is not a safer control.

**The classification on the finding comes from the BCR.** `BoundaryFlow::Ingress` carries none, and conditions 10 and 11 are reached only once provenance is established — so a custody record exists and `b.classification` is the path. This was the one value Rev 1.29's §5.4 table omitted; it is recorded here and in the table below.

##### What the built half proves, and what no amount of building would

**It proves that admitted content matches a signed declaration, within a field set a DAP signed in advance.** Both halves of that are attributable and tamper-evident, now doubly so.

**It does not prove the declaration is minimal, and nothing in this repository could.** A DAP who signs a `PurposeFieldPolicy` listing every field the organization holds gets it enforced faithfully — the gate working exactly as designed while OQGF-P-11.2's substance goes unmet. Whether a field is *necessary* for a purpose is answerable only from why the data is collected; **it is not a predicate over any value the evaluator holds**, which is why it is recorded as an assessment finding rather than as unfinished work.

**This is the residual shape of the self-declared Capability Envelope (P-12.3) and the self-declared custody model (R-6.2)**, and AMD-018 §AMD.2.1 governs all three: *a declared model that overstates what is actually achieved is a conformance failure, not a documentation defect.* Testing it is an assessor's work against a live deployment.

**What changed is what an over-broad scope costs.** Before, minimization was **unstated**: nothing declared a field scope, so nothing could be inconsistent with one. Now an over-broad scope is a **signed, dated, DAP-attributed artifact in the genome**, reviewable in a diff and enumerable at assessment. That is the same argument §6.11 makes for the custody declaration and it carries the same limit: **the declaration is not the control, and a field containing a claim is not evidence for the claim.**

##### The heuristic backstop remains future work, and not for convenience

The content sentinel would flag a declaration that *lies* — a tag listing three fields over content carrying five. It is not built, and the reason is the same one that reshaped the deterministic check: **no content reaches the barrier.** `BoundaryFlow` carries `DatumRef`, an opaque identifier. The sentinel has no more access to content here than the gate does.

**So this is unbuildable against the current surface, not deferred because it is expensive.** The distinction matters because the two look identical in a residual list. A deferred item is waiting on effort; this one is waiting on a surface that does not exist and whose creation OQGF-I-12 constrains — content inference belongs to the trained layer, and HEIMDALL is where it would live if the observations carried content. Recording it as future work without the reason would invite someone to schedule it.

##### §5.4 — every value the placed rules read, and how reachability was verified

*(Rev 1.29.)* Verified against the committed tree at `e68a235`, not against earlier revisions. **Three rows record surfaces that do not exist and must be placed before the rule can be written** — recorded as such rather than assumed into the design.

| Placed rule | Value it reads | Path — named accessor, named crate | How verified |
|---|---|---|---|
| Condition 10/11 apply at all | the context class | `BoundaryFlow::Ingress { context }` — `brokkr-core` | **observed**: the variant is committed and `Huth::ingress` already matches on it |
| Condition 10/11 apply at all | the declared purpose | `flow.personal.as_ref()?.purpose` — `brokkr-core` | **observed**: `PersonalDataTag { purpose, retention }` is committed and the ingress path already reads it (Rev 1.7) |
| Condition 10 | the allowed field set | `PurposeFieldResolver::allowed_fields` — `brokkr-barrier`, injected | **placed here.** The seam pattern is committed and observed: `Huth<C: EndpointCeiling, A: AcceptanceResolver>` is already generic over two such traits, and `brokkr-barrier/Cargo.toml` confirms no `brokkr-genome` dependency, so a third preserves I-5 |
| Condition 11 | the present field set | `flow.personal.as_ref()?.fields` — `brokkr-core` | **placed here.** `PersonalDataTag` has no `fields` member today; the field is new |
| Condition 11 refusal | the condition variant | `BarrierCondition::PersonalDataFieldOutOfScope` — `brokkr-core` | **placed here.** The eight committed variants were enumerated and none expresses it |
| The declaration is signed | the tag inside the BCR | `bcr_signed_content` → `write_personal` — `brokkr-barrier` | **observed**: `canonical.rs:90` encodes purpose and retention today; encoding `fields` extends it, and **invalidates existing personal-tag BCRs** |
| Condition 11 refusal | the finding's classification | `bcr.classification` on the established record — `brokkr-core` | **observed at implementation.** `BoundaryFlow::Ingress` carries no classification and `BarrierFinding` requires one; conditions 10/11 are reached only once provenance is established, so a BCR exists. Omitted from this table at Rev 1.29 and recorded by GAP-2026-09-10-001 §5 |
| **Not placed** | the fields admitted content actually carries | — | **No path, and deliberately not created.** `BoundaryFlow` carries `DatumRef`, a `string_id!` opaque identifier (`ids.rs:41`); no content reaches the evaluator. Creating one would put content inference inside a Deterministic Gate, which OQGF-I-12 forbids |
| **Not placed** | whether a declared field set is *minimal* | — | **Not decidable from any value.** A governance judgment; an assessment finding, not a predicate |

**The last two rows are the point of this table.** One rule is unbuildable and is not designed around; one is not a rule at all and is recorded as assessment. Rev 1.21 placed a predicate against a tier no code could reach and Rev 1.24 placed rules against fields the API does not expose; **both were caught at Step 0 by a builder rather than at drafting, which is the failure this table exists to prevent.**
...
*(Rev 1.19 recorded the stream live; Rev 1.31 records that it carries a comparison which cannot fail.* The orchestrator emits `Observation::Hop { executed: Some(..) }` after a successful execution (`brokkr-cli/src/lib.rs:897`) and `executed: None` when an **authorized** action fails to run (`:879`, 14-FIX F-20) — **and `executed` is `action.clone()`, the same value as `authorized`.** `Tool::execute` returns `ToolOutcome { output: String }`, opaque to the spine and compared to nothing, so nothing derives what the execution actually did. **`reconciliation_deviation` therefore cannot fire from a production emission.**

**What this leaves standing, and what it does not.** The mechanism is real: `Heimdall::observe` runs the comparison on every hop, a deviation raises a Signal through the graded-response path (OQGF-P-7), and the A.6.1 statistical-reconciliation-failure trigger is wired. Fed a genuine deviation — as the sentinel tests feed it with supplied pairs — it fires. **What has never happened, and cannot happen on the current emissions, is a deviation arising from a real execution.** `executed: None` remains meaningful and is the one live signal the stream genuinely carries: an authorized action that did not run is visible, which is what F-20 was for.

**OQGF-M-12 therefore moves to PARTIAL** (§14) — mechanism built and tested, live input vacuous — and **OQGF-M-6's fifth factor cannot be measured from it**, which is why no counter is placed (§6.2).*
...
- **The host-harm rate is not measured at runtime at all, and the under-counting residual describes the design rather than the running system.** *(New in Rev 1.11 as "a lower bound, biased toward looking safe"; corrected at Rev 1.31.)* **`record_governed_action` and `roll_window` are called only in `brokkr-sentinel/tests/sentinel.rs` (lines 534, 544, 551, 561) and nowhere in `brokkr-cli`.** So in a running BROKKR the denominator stays `0`, the window never rolls, `window_rate(0, 0)` returns `NaN` forever, the over-bound streak never advances, and **autoimmunity (OQGF-P-5a) can never be detected.** The prior characterisation described a rate that is measured and undercounts; there is no numerator, no denominator, and no window. The design's residual remains true **of the design** and is restated so it is not lost in the correction: the numerator counts only *confirmed* incidents, so once wired the rate will under-state real host harm and always in the same direction — **a rising confirmed rate is real evidence, a low one is weak evidence, and today there is no rate at all, which is weaker still.** What is **not** wrong is the `NaN` discipline, and it is why this is a recorded absence rather than a silent reassurance: `window_rate` refuses `0.0` from an empty window because *"a rate of `0.0` would claim safety from none."* **The type is honest; the wiring is missing** — the same shape as OQGF-M-6's fifth factor, a counter with no producer wiring, which §6.2 records as having now occurred three times.
...
- **A canonical-encoding change reaches the audit chain through `AuditEvent::BarrierCrossing`, and a placed revision asserted otherwise.** *(New at Rev 1.30; disposes GAP-2026-09-10-001.)* Rev 1.29 bounded the `PersonalDataTag` cost on the claim that *"a BCR is not an `AuditRecord`, `record_signed_content` does not encode it."* It does — by two paths, `BarrierCrossing` and `RecordedInput::Shredded` — because **OQGF-I-13 requires every barrier decision recorded in Organ 5**, and the event carries a whole `BoundaryFlow`. **The residual this leaves is not the encoding but the class of claim:** an assertion about a consequence in committed code, checkable and unchecked, which no existing rule reaches (§5.4 governs a rule's inputs, §5.3 verdicts, §7 the builder's summaries). It was caught the way the previous three were — a builder grounding a placed design before writing code. **No rule is added for it**; one is recommended and labelled in §6.5 rather than placed. Cost as corrected: zero now (SAGA in memory, no `serde`, nothing serialised), **not zero once SAGA persists.**
...
- **Minimization enforces conformity to a signed declaration; nothing proves the declaration is minimal, and nothing in this repository could.** *(New in Rev 1.7; rewritten at Rev 1.29; rewritten again at Rev 1.30, when the design was built.)* The **declaration half is code**: `FieldName`, `PurposeFieldPolicy` in `PolicyRegister`, `PersonalDataTag::fields`, `BarrierCondition::PersonalDataFieldOutOfScope`, the `PurposeFieldResolver` seam, and ingress conditions 10 and 11, each under a named test (§6.5). **The verdict does not move**, because what remains is not unfinished work:
  - **An over-broad declaration is enforced faithfully.** A DAP who signs a policy listing every field the organization holds gets exactly that gated. Whether a field is *necessary* for a purpose is answerable only from why the data is collected — **not a predicate over any value the evaluator holds** — so this is an **assessment finding**, the residual shape of P-12.3's self-declared envelope and R-6.2's self-declared custody model, governed by AMD-018 §AMD.2.1. What changed is that an over-broad scope is now a signed, dated, diffable artifact where minimization was previously unstated.
  - **A declaration that lies is outside the deterministic check, and the backstop that would catch it is unbuildable rather than deferred.** The gate enforces that a datum matches what it *says* it carries. A tag listing three fields over content carrying five is the content sentinel's business (OQGF-I-12) — and **no content reaches the barrier**, so the sentinel has no more access than the gate. Recorded as future work *with the reason*, because a deferred item and an unbuildable one look identical in a residual list and only one is waiting on effort.
  - **Semantic over-collection inside a permitted field is reached by no field-level control at all** — a permitted `notes` field carrying a medical history. The covert-channel residual in a new place.
...
- **The vendor reconciliation pass rate is unmeasured, and measuring it from the current stream would fabricate it.** *(New in Rev 1.4; rewritten at Rev 1.31; disposes GAP-2026-09-15-001.)* OQGF-M-6's fifth factor is the output of OQGF-M-12 cross-hop reconciliation — a reading Rev 1.4 placed and Rev 1.31 does not reopen, since the alternative M-3 reading would make the factor `n.a.` and the problem vanish. **The orchestrator clones `authorized` into `executed` (`brokkr-cli/src/lib.rs:897`), so every executed hop is a pass by construction and a counter would return exactly 1.0.** Placing one would put a **fabricated 100% into a signed register the OQGF-G-4 gate reads** — worse than the inert `Score(0)` it replaced, because an absence becomes an affirmative false claim indistinguishable at assessment from a measurement (§6.2). **Closing it needs an effect description the spine can compare against an `Action`**, touching `brokkr-core`, `brokkr-tools`, and `brokkr-cli` — scoped in §6.2, not designed.
## 14. Traceability
...
| **OQGF-M-12 (cross-hop reconciliation)** | **PARTIAL — moved BACKWARD from `SATISFIED` at Rev 1.31, the first backward verdict movement in this document.** *(Was `SATISFIED` at Rev 1.19; disposes GAP-2026-09-15-001.)* **The mechanism is built and tested**: `Heimdall::observe` runs the authorized-vs-executed comparison on every hop, a deviation raises a Signal through the OQGF-P-7 path, the A.6.1 reconciliation-failure trigger is wired, and the sentinel tests demonstrate it firing on supplied pairs. **The live input is vacuous**: `brokkr-cli/src/lib.rs:897` emits `executed: Some(action.clone())` — a clone of `authorized` — and `ToolOutcome` carries an opaque `String` that is never compared, so **no production emission can produce a deviation** and the mechanism has never had the opportunity to fire. Rev 1.19 checked that the stream existed and that F-20's `executed: None` was emitted; both were true and both were checked. **What went unchecked is whether the two sides of the comparison were independently derived** — GAP-2026-09-10-001's shape, a real question answered accurately beside the one that decides the answer. `executed: None` remains a genuine live signal (an authorized action that did not run). Closing the rest needs an **effect description** (§6.2), touching `brokkr-core`, `brokkr-tools`, and `brokkr-cli` |
...
| **OQGF-P-11.1, P-11.3, P-11.4 (AMD-009)** | **`brokkr-core::barrier::PersonalDataTag` — orthogonal to `Classification`; carried on both the flow and the BCR; egress condition 9 and the Privileged-Context ingress rule (§6.5, Rev 1.7). PARTIAL for P-11.2 — **Rev 1.30 records it built and the verdict does not move.** The **declaration half is code**: `FieldName`, `PurposeFieldPolicy` in `PolicyRegister` reached through a `PurposeFieldResolver` seam (preserving I-5), `PersonalDataTag::fields` inside both the BCR's signed content **and** the audit chain's, `BarrierCondition::PersonalDataFieldOutOfScope`, and ingress conditions 10 and 11 — five named tests, mutation-checked. An unresolvable purpose **refuses**, since permitting would make the control removable by declaring an unregistered purpose. **The remaining half is not unfinished work:** whether a signed field list is *minimal* is answerable only from why the data is collected and is not a predicate over any value the evaluator holds — an **assessment finding**, the residual shape of P-12.3 and R-6.2 (§13). P-11.5 crypto-shredding is `brokkr-crypto` (Phase 2); P-11.6/P-11.7 are SAGA (Phase 7)** |
...
**Rev 1.31 moves OQGF-M-12 backward and holds OQGF-M-6 still, and both directions had a temptation worth naming.**

**M-12 moves backward, and the temptation was not to.** A backward move is the first in this document, it retracts what a prior revision recorded, and every incentive points at leaving it: the mechanism genuinely is built, the tests genuinely pass, and *"SATISFIED, with a residual noted in §13"* was available and would have read as rigour. **It would also have been false.** OQGF-M-12 requires the executed action to be reconciled against the declared intent, and what is reconciled is a value against a copy of itself. §5.3 permits a backward move and warns that **a record whose verdicts only ever improve is not being audited** — a sentence with no instance until now, which is itself the finding. **A register that only ever improves is not being read.**

**The opposite temptation is equally real: a verdict must not move backward for drama.** A backward move reads as rigour the way a forward move reads as progress, and neither is evidence. M-12 moves because of a specific checkable fact at a named line — `executed` is `action.clone()` — which makes the requirement's central comparison unable to fail. It does **not** move to `ABSENT`: the mechanism exists, is tested, fires when fed a real deviation, and `executed: None` carries a genuine signal. **`Partial` is the honest distance, not the dramatic one.**

**M-6 does not move, in either direction.** It stays `PARTIAL` — four factors declared, `data_handling` retained as a tightening, the fifth unmeasured. **Not forward**: nothing was measured, and this revision explicitly declines the counter that would have produced a number. **Not backward either**, and that temptation was present: discovering the fifth factor is not merely unmeasured but *unmeasurable from the current stream* feels like a deterioration. It is not. The verdict was already `PARTIAL` because the factor awaits measurement; **learning why the wait is longer than assumed changes the residual's content, not the verdict's category.** The record now says the factor cannot be honestly measured until the executor reports what it did — information, not a downgrade.

**Rev 1.30 changes no verdict in this table, and the temptation was the strongest it has been.** P-11.2 is the oldest open half in this group — Rev 1.7 recorded it — and unlike the last six revisions there is now **working, tested, mutation-checked code** behind it. A revision that ships an implementation reads like a closure, and the row is one word away from looking finished. **It is not finished, and the unfinished part is not work:** a DAP who signs an over-broad field list gets it enforced faithfully, and no predicate over any value the evaluator holds decides whether a field is *necessary* for a purpose. **A requirement whose remaining half is an assessment finding does not become satisfied by building the half that is code.** §5.3 warns that a record whose verdicts only improve is not being audited; the sharper form here is that a verdict must not move because a revision finally had something to show.

**Rev 1.29 changes no verdict in this table, and the temptation this time was to move a row that has been open since Rev 1.7.** P-11.2 is the oldest unaddressed half of a requirement in the A-3/P-11 group, the design is concrete, and it would be easy to read "minimization is now designed" as "minimization is now handled." It is not: **nothing is implemented**, and the half that a full implementation would still not reach — whether a signed field list is actually minimal — is not a predicate at all. **A design that names its own residual is not a residual closed.** §5.3 warns that a record whose verdicts only improve is not being audited; the corollary applied here is that a verdict must not move because a revision produced a satisfying document.
...
**Rev 1.32 — 22 September 2026. Disposes GAP-2026-09-18-001. Removes a field, declines a rule, and moves no verdict. The removed field supplied no property; the declined rule is recorded as a decision rather than left to lapse.**

- **§6.2 — `VendorTrustScore::signature` is removed.** From Rev 1.2 to Rev 1.31 the type carried a `DualSignature` that **nothing computed, nothing verified, and for which no signed content was ever defined.** Four checks at HEAD: `brokkr-genome/src/canonical.rs` exports exactly seven signed-content functions and **none for the trust score**, so there was nothing for the field to be a signature *over*; a workspace grep for `trust_score` conjoined with `sig` returns **no hit at all**; predicate 2 verifies six register signatures plus the genome's and **not this one**; and both fixtures filled it with a placeholder.
- **The property a reader would infer from it is real, and the register supplies it.** `write_vendor_trust_score` → `write_model_endpoint` → `endpoints_signed_content`, so **the whole score is inside the bytes the endpoints register signature covers**, and predicate 2 verifies that signature against the DAP root. The score was tamper-evident and DAP-attributable before this revision and remains so after it. **The field added the shape of an independent attestation with no mechanism behind it.**
- **This is the OQGF-R-6.2 shape inverted, and that framing is the useful half.** There, a custody posture was asserted for eighteen revisions with **no field** behind it — a claim with nothing to check. Here, **a field with no claim behind it.** Both mislead in the same direction, and **the second is easier to miss because a field *looks* like implementation.** The encoder's comment was honest about it and said so plainly; **the code was not lying — the type misled, and so did §6.2's unqualified snippet, which is how an assessor reads a type.**
- **§6.2 — what the removal is *not*, recorded so it is not read as a judgment it is not.** It is **not** a finding that a separate signer is unwanted. A deployment where vendor risk is signed by a party other than the endpoints-register signer — a procurement function distinct from whoever configures endpoints, plausible for a federal buyer — is a real model. **It does not exist today, and a field with no verification site is not a hook for it:** an unverified field does not reserve a future control, it asserts a present one. If that model arises it is **placed then, as a design, with a signed-content function and a verifying predicate**, because a second attestation is worth exactly what its verification site is worth. Nothing is foreclosed by the removal and nothing was reserved by the field.
- **§6.2 — the cost, checked at HEAD rather than repeated.** Removal moves `endpoints_signed_content` and `genome_signed_content` for the **third consecutive revision** (Rev 1.29's `PersonalDataTag`, Rev 1.31's `FactorEvidence`, this). Three claims bound it at zero **and each was re-verified, because the Rev 1.29 cost paragraph was wrong for exactly this reason**: SAGA holds `records: Vec<AuditRecord>` in memory; **no workspace crate declares a `serde` dependency at all**, so nothing is serialised and no stored digest can move; and the promotion fixture computes its signature over `endpoints_signed_content` at test time, so the encoding change moves *with* the fixture rather than breaking it. **Zero now is not zero later.**
- **§6.2 — the audit chain does not move, and it is answered structurally rather than by inspection.** `write_genome_promotion` encodes `version` and the *declared* `corpus_digest`, neither derived from a register encoding — and, decisively, `brokkr-audit` depends only on `brokkr-core` and `brokkr-crypto` (I-5), **so `VendorTrustScore` is unreachable from `record_signed_content` by any path.** That is the distinction GAP-2026-09-10-001 turned on — *is this type inside an audit record?* rather than *is this type an audit record?* — and a dependency direction cannot be answered wrongly by inspecting one encoder and missing a second.
- **§6.2 — a second staleness, found by correcting the first.** The type snippet had never been updated to show `evidence: FactorEvidence`, placed by Rev 1.31. **It therefore misdescribed the type in two directions at once** — over-stating what was attested, under-stating what was recorded — in the one artifact an assessor reads to evaluate OQGF-M-6. Both are corrected.
- **§6.2 — the dated Rev 1.31 §5.4 table is not rewritten.** Its predicate-8 row enumerates the struct's fields as they then stood, `signature` among them, under a heading stating it was *"verified against the committed tree at `53f2c33`."* **A dated record of what was checked when is history, not current text** (§8). A reader comparing that row against the corrected snippet will find they disagree, which is correct: they describe different revisions and each says which.
- **§6.2 — §5.4 places no obligation on this revision, and the table says so.** §5.4 governs a rule placed against a value; **Rev 1.32 places no rule.** What replaces it is the enumeration a removal actually needs: every site that read, wrote, encoded, or documented the field — five files workspace-wide, plus two documentation sites, plus one dated record left alone.

- **§6.5 — a one-line pointer, and the original sentence is not rewritten.** Rev 1.30 §6.5 closed with *"Recommended, not placed"*, which remains true of Rev 1.30 and is left exactly as placed (§8). A pointer is **appended after it** recording that the recommendation was **declined at Rev 1.32** and naming GAP-2026-09-18-001 §7, which carries the text verbatim with the argument on both sides. **A reader of §6.5 would otherwise see an open recommendation and no disposition** — the exact failure the decline was recorded to prevent, one section away from the record of it.
- **§14 — a one-line citation correction.** The OQGF-M-6 row cited **`brokkr-genome::VendorTrustScore`**; the type is declared in **`brokkr-core::genome`** and merely encoded by `brokkr-genome`. Corrected in place. **The verdict does not move and no other word of the row changes** — it is a wrong crate in a traceability row, which is the one place an assessor goes to find the type.

**Ruling 2 — the consequence-claim rule recommended at Rev 1.30 §6.5 is DECLINED, on the record.**

- **What was recommended.** That *a revision asserting a consequence about committed code names the file and symbol it checked* — the §5.4 discipline extended from a placed rule's *inputs* to a revision's *consequence-claims*. Raised parenthetically at Rev 1.30 §6.5 and carried, with the argument for and against at equal length, in GAP-2026-09-18-001 §7.
- **Why it is declined, and the reasoning is recorded so the decision is reviewable rather than merely final.** **First: every gap in this repository was caught by a practice that is already mandatory** — a builder grounding a placed design against the tree at Step 0. That practice found GAP-2026-09-07-001, -09-08-001, -09-10-001, -09-15-001, and -09-18-001. **A rule that duplicates a working practice adds a box to tick and does not add a check.** **Second: a rule per incident is how a framework acquires ceremony instead of control**, and ceremony is not neutral — it consumes the attention a real check needs. **Third, and this is CLAUDE.md §12.1's argument applied to a build rule rather than an invariant:** a rule enforcing the *presence of a claim* — here, a named file and symbol — while the claim's *truth* lives somewhere the rule cannot see **teaches a reader that the rule set covers something it does not.** A revision could name an encoder, name it wrongly or incompletely, and satisfy the rule while committing the Rev 1.29 error exactly.
- **The argument against the decline is not weak and is not dismissed.** §5.4 exists because *naming the pattern did not prevent repeating it* — awareness was maximal and insufficient. That argument applies to consequence-claims as much as to inputs, and the Rev 1.29 error is a real instance of the gap. **The judgment is about how much process this repository can carry before the process becomes the work**, and it is a DAP judgment. It is decided against, and it may be revisited if a consequence-claim error recurs after a Step-0 check that did not catch it.
- **The declined text is preserved.** Rev 1.30 §6.5 records it verbatim as *"Recommended, not placed"*, which remains true; GAP-2026-09-18-001 §7 records it verbatim with both sides. **Nothing is deleted** (§8), and a future reader sees a considered refusal rather than an idea that evaporated.

**What Rev 1.32 does not do.** It moves no verdict. **OQGF-M-6 stays `PARTIAL` for Rev 1.31's reason** — the fifth factor is unmeasured and unmeasurable from the current stream — and **nothing beneath that row moved**: the five corpus factors, the retained `data_handling` tightening, the `evidence` pair, predicate 4's staleness arithmetic, and predicate 8's honest-state exemption all stand as Rev 1.31 left them. **No verdict ever rested on the removed field:** OQGF-M-6 requires five factors and a quarterly review and imposes **no attestation obligation on the score itself**. **I-10 is unaffected** — it requires a signed *Endpoint Registry*, which is the signature that carries the score. §13 and §14 are untouched, because neither references the field. It places no type, no predicate, and no invariant, and it closes no residual. **It removes a claim the implementation never made, and records a decision that would otherwise have lapsed.**

**Self-interest labelling per §4.** **Removal is the cheaper of the two dispositions GAP-2026-09-18-001 offered, and the builder recommended it** — a field deletion and two fixture edits, against a new encoder, a signature computation, a predicate, and tests. The DAP placed it, but a reader weighing this revision should know the cheaper option was also the recommended one, and that **the entire basis of the recommendation is falsifiable in two lines**: read the encoder chain from `write_vendor_trust_score` to `endpoints_signed_content` and confirm the register already covers the score. If it does not, the removal is wrong. **The declined rule cuts the other way and is noted for balance:** declining it spares the builder a named-evidence check on every future revision, and that is a saving, recorded so it is visible rather than buried in the reasoning for the decline.

**Rev 1.31 — 15 September 2026. Disposes GAP-2026-09-15-001. Moves OQGF-M-12 backward — the first backward verdict movement in this document — declines to place a counter, and places the `Score` question.**

- **§6.2, §6.7, §14 — OQGF-M-12 moves `SATISFIED` → `PARTIAL`.** Rev 1.19 moved it forward on the strength of the live reconciliation stream existing. **The stream exists and reconciles the authorized action against a clone of itself**: `brokkr-cli/src/lib.rs:897` emits `executed: Some(action.clone())`, and `ToolOutcome { output: String }` is *"opaque to the governance spine"* and compared to nothing. **`reconciliation_deviation` cannot fire from any production emission.** The mechanism is real and tested — it fires on supplied pairs — and has never had the opportunity to fire on a real execution.
- **Rev 1.19 was not careless, and the shape of its miss is the useful part.** It verified that the stream existed and that F-20's `executed: None` was emitted. Both true, both checkable, both checked. **What it did not check is whether the two sides of the comparison were independently derived** — GAP-2026-09-10-001's shape exactly: a real question answered accurately, next to the question that decides the answer.
- **§14 — the first backward move, and both temptations are named.** §5.3 permits a backward verdict and warns that *"a conformance record whose verdicts only ever improve is not being audited"* — **a sentence with no instance until now.** The temptation was *not* to move it: `SATISFIED` with a §13 residual was available and would have read as rigour. The opposite temptation is equally real — **a verdict must not move backward for drama any more than forward for progress** — which is why M-12 goes to `PARTIAL` and not `ABSENT`: the mechanism exists, is tested, and `executed: None` carries a genuine signal.
- **§6.2 — no reconciliation counter is placed, and the counter was the easy part.** `HeimdallState` already holds `governed`/`incidents` with `roll_window()`; a pass/fail pair is a small testable change. **The reason to refuse is what the number would assert.** Over a degenerate input it returns exactly 1.0, and a computed `Score(100)` in a signed register is **worse than the inert `Score(0)` it would replace**: `Score(0)` asserts nothing today because nothing computes or reads the field, while a computed 100% is an affirmative false claim, signed, DAP-attributed, gate-read, and **indistinguishable at assessment from a measurement.** The **R-6.2 defect class** (AMD-018 §AMD.2.1) and §7's fabricated-completion prohibition, relocated into a register where it carries more authority.
- **§6.2 — the reading of the fifth factor is not reopened.** M-6's phrase is *"statistical reconciliation pass rate"*, which is OQGF-M-3's vocabulary, and M-3 is `n.a.` for BROKKR. **Taking the M-3 reading would make the factor vanish and this problem with it** — reading a requirement down until it disappears, refused since Rev 1.2, and never more convenient than here.
- **§6.2 — the `Score` question, placed.** `Score` is `u8`, 0..=100, higher is better, and **has no value meaning "unmeasured"**; `Score(0)` is the worst score, not a neutral one. **The right instinct is already committed one crate away**: `window_rate` returns `NaN` rather than `0.0` for an empty window because *"a rate of `0.0` would claim safety from none."* **The host-harm rate can say "no evidence"; a trust score cannot.** `Option<Score>` is **refused** — it makes absence representable while saying nothing about **how much** evidence a present score rests on, and `Some(Score(100))` from one observation and from ten thousand are indistinguishable. **`FactorEvidence { observations, measured }` is placed**, with `measured` deliberately distinct from `reviewed`: a review can restate an old measurement, and conflating them would let a fresh review launder stale evidence.
- **§6.2 — promotion-gate predicate 8, and its direction is the whole of its safety.** It refuses a genome whose `reconciliation_pass_rate` is non-zero while `observations == 0` — **a factor claiming a measurement it has no evidence for.** It does **not** refuse an unmeasured factor: BROKKR's own genome is in that state and must stay promotable. **A gate refusing the honest state and admitting the dishonest one would be precisely backwards.** `Score(0)` paired with `observations: 0` becomes the honest encoding of "unmeasured", and **the pair, not either half, carries the meaning.**
- **§6.2 — the fixtures are inert today and one producer away from lying.** `promotion.rs:94` and `negative_invariants.rs:520` carry `Score(0)`, which asserts nothing while nothing computes the field. **The moment a producer exists it asserts a 0% pass rate from no observations** — the worst measurement obtainable, claimed from nothing. What they should then carry is `Score(0)` paired with `observations: 0`. **No fixture is changed here**, because nothing is implemented; the point is that nobody editing the producer would think to look at them.
- **§13 — §13's own characterisation of the host-harm rate is corrected.** It said *"a lower bound, biased toward looking safe"*, which describes a rate that is measured and undercounts. **`record_governed_action` and `roll_window` are called only in `brokkr-sentinel/tests/sentinel.rs` (534, 544, 551, 561) and nowhere in `brokkr-cli`**, so the denominator stays `0`, the window never rolls, `window_rate(0,0)` is `NaN` forever, and **autoimmunity can never be detected.** The under-counting residual remains true *of the design* and is restated rather than dropped. **The `NaN` discipline is not what is wrong** — it is why this is a recorded absence rather than a silent reassurance. **The type is honest; the wiring is missing.**
- **§6.2, §13 — a counter with no producer wiring, three times.** `reconciliation_pass_rate` (no accumulator), the host-harm counters (test-only callers), and the counter this revision declines to add. **Recorded as an observation for the next design, not as a rule**: a counter incrementing inside a method the orchestrator already calls — `observe` — is wired by construction; one behind a method it must remember to call is wired by discipline. Per GAP-2026-09-10-001, a rule per incident is ceremony rather than control.
- **§6.2 — closing the fifth factor is scoped, not designed.** **The executor must report what it did, distinguishably from what it was told to do.** `ToolOutcome`'s opacity is the **correct** posture — a spine that parsed tool output for meaning would take instruction from the thing it governs — and is exactly why no reconcilable value exists. An **effect description** in the governance vocabulary touches `brokkr-core` (the type), `brokkr-tools` (every `Tool::execute`, **where the cost lives**), and `brokkr-cli` (derive rather than clone). The hard question is what an effect description *is* for a file write versus a shell command versus a network call, and whether a tool that cannot honestly describe its effect should be refusable at the genome. **That is a placement, and the builder does not place.**
- **§6.2's §5.4 table carries a row of a kind this document has not had before.** A value recorded as **reachable and still not placed** — every prior "not placed" row was blocked by an absent path. **Reachability was the right question and is not the only one**: *can the evaluator obtain this value* was answered yes, and *does the value mean anything* was answered no.

**Self-interest labelling per §4.** **Declining the counter is the choice that saves me the most work, and it is the one a reader should check hardest.** The accumulator and window are easy, the `governed`/`incidents`/`roll_window` precedent is committed, and a passing implementation could have shipped the same day. **The finding is falsifiable in one line: read `brokkr-cli/src/lib.rs:897` and see whether `executed` is derived or cloned.** Two entries move against my interest and are noted so the balance is visible: **moving M-12 backward** retracts a verdict recorded in my own prior work and invites the question of what else was moved forward too easily, and **placing `FactorEvidence` plus predicate 8** adds a type, a gate predicate, and fixture obligations to a revision that could have recorded the problem and placed nothing.

**What Rev 1.31 does not do.** It implements nothing — no `FactorEvidence`, no predicate 8, no counter, and no fixture is changed. It does not close OQGF-M-6's fifth factor or claim a path to closing it without the effect-description change, which it scopes rather than designs. **It does not move OQGF-M-6 in either direction.** **Drafted for DAP review; not placed, not committed.**

**Rev 1.30 — 11 September 2026. Disposes GAP-2026-09-10-001 by correcting a false claim this document made, and records OQGF-P-11.2's implemented state. Places no mechanism; moves no verdict.**

- **§6.5 — the corrected claim.** Rev 1.29 bounded the `PersonalDataTag` cost with: *"a BCR is not an `AuditRecord`, `record_signed_content` does not encode it, and no `prev` digest covers it."* **The first clause is true; the other two are false.** `PersonalDataTag` reaches `record_signed_content` by **two** paths, both in `brokkr-audit/src/canonical.rs` — `AuditEvent::BarrierCrossing → write_flow → write_personal → write_personal_tag`, and `RecordedInput::Shredded → write_personal_tag`. **`brokkr-audit` keeps its own copy of the encoder**, distinct from the barrier's; Rev 1.29 named the barrier's and did not look for a second.
- **Why the sentence read as correct, which is the part worth keeping.** It answers *is a BCR an `AuditRecord`?* accurately — no. The question that determines the cost is *is a BCR **inside** one?* — yes, because **OQGF-I-13 requires it**, and `BarrierCrossing` carries a whole `BoundaryFlow`. **The architecture's own requirement puts the tag in the chain, two sections from the paragraph denying it.**
- **§6.5 — the corrected cost, and why correcting it matters when the number is zero.** The change moves `bcr_signed_content` **and** `record_signed_content`. **Zero today**: SAGA holds records in memory, `brokkr-audit` has no `serde` dependency and serialises nothing, and the four affected fixtures move with the encoding rather than break. **Not zero once SAGA persists** — and the false sentence would have been read by whoever assesses the next encoding change, under exactly the condition that makes it expensive.
- **§6.5 — the property is delivered more strongly than Rev 1.29 claimed.** The declaration is covered by the BCR's dual-family signature **and** the audit chain's linkage and generation signatures — **two attestations, not one.** The revision undersold its guarantee while misstating its blast radius, and both halves are recorded: overstating is the familiar failure, understating a guarantee while misstating its cost is the same carelessness pointed the other way.
- **§6.5, §13 — no rule is invented for it.** §5.4 governs a placed rule's *inputs* and every value here is reachable; §5.3 governs verdicts; §7 binds the *builder's* summaries and this claim was the DAP's, in a placed revision. Rev 1.10's discipline is the principle at stake and lives as a constraint on the encoding, not a check over revisions touching types that feed it. **It was caught the way the previous three were — a builder grounding a placed design before writing code, which is already mandatory.** A rule per incident is how a framework acquires ceremony instead of control. One narrow rule is **recommended and labelled, not placed** (§6.5).
- **§6.5, §14 — P-11.2's declaration half is built.** `FieldName`, `PurposeFieldPolicy` in `PolicyRegister`, `PersonalDataTag::fields`, `BarrierCondition::PersonalDataFieldOutOfScope`, the `PurposeFieldResolver` seam, and ingress conditions 10 and 11 — five named tests. **An unresolvable purpose refuses**, because permitting would make the control removable by declaring a purpose nobody registered: not a weaker control but an absent one.
- **§6.5 — one test exists to keep the gate from being too tight.** A non-privileged context is **not** gated. OQGF-P-11.2 governs admission to a Privileged Context, and refusing elsewhere would be stricter than the requirement — the direction OQGF-P-1 treats as a governance failure of equal standing to a missed threat.
- **§6.5, §13, §14 — the verdict does not move, and the remaining half is not work.** Whether a signed field list is *minimal* is answerable only from why the data is collected; it is **not a predicate over any value the evaluator holds.** An assessment finding, the residual shape of P-12.3's self-declared envelope and R-6.2's self-declared custody model. **What changed is that an over-broad scope is now a signed, dated, diffable artifact where minimization was previously unstated.**
- **§6.5, §13 — the heuristic backstop is unbuildable, not deferred.** No content reaches the barrier, so the sentinel has no more access than the gate. Recorded **with the reason**, because a deferred item and an unbuildable one look identical in a residual list and only one is waiting on effort.
- **§1.5 — the open-item picture, checked item by item, and one entry corrects the convenient summary of it.** Seven of the eight open Enhanced items wait on deployment, organization, or assessment — R-6.2's hardware boundary and dual control (element 3 **is built**), P-12.3's OS-level confinement (the probe **is built**), A-3 clause 2's responder, M-1's attestation issuer, P-12.6's sub-agents, P-12.8's independent sensor, P-11.2's assessor. **OQGF-M-6 is the exception and needs code here**: `reconciliation_pass_rate` has **no producer** — declared, encoded, `Score(0)` in every fixture, and no accumulator anywhere in `brokkr-sentinel`. *"Nothing is waiting on code"* is 87% true, reads clean, and is the shape of claim this revision exists to correct.

**Self-interest labelling per §4.** This revision records and corrects, so it creates no implementation work — and **the entry that most reduces my work is recording P-11.2's remaining half as an assessment finding rather than as a backlog item**, which I am the party proposing. The reason stands independently and should be weighed on it: no predicate over any value the evaluator holds decides whether a field is *necessary* for a purpose, which is why §6.5's §5.4 table records it as "not a rule" rather than "not yet placed". **Two entries move against my interest and are noted so the balance is visible:** recommending a rule about consequence-claims would cost the builder work on every future revision and save none, and recording M-6 as code-blocked adds a build task to a list I could have left reading as uniformly deployment-blocked.

**What Rev 1.30 does not do.** It places no type, gate, or predicate and implements nothing. It moves no verdict — P-11.2 stays `PARTIAL`, and the half that remains is not closable by code. It does not claim minimization is enforced: it claims a signed declaration is enforced, which is a smaller and true thing. **Drafted for DAP review; not placed, not committed.**

**Rev 1.29 — 10 September 2026. Places the OQGF-P-11.2 minimization design (§6.5). Implements nothing; P-11.2 remains PARTIAL, and would move only partly even fully built.**

- **Step 0 changed the design's shape, and the record says so rather than presenting the result as what was asked for.** The natural rule — inspect admitted content, refuse fields the Purpose does not need — **is unbuildable and would be wrong if it were buildable.** Unbuildable: `Barrier::evaluate` takes a `BoundaryFlow`, whose `Ingress` variant carries `DatumRef` — a `string_id!` newtype (`ids.rs:41`) documented as *a reference to* a datum — so **no content reaches the evaluator and no accessor would yield any.** Wrong: content inference is what OQGF-I-12 designates *Heuristic*, and **a Deterministic Gate SHALL NOT take its input from a heuristic one.** Giving the barrier content would collapse the innate/adaptive boundary AMD-002 exists to hold.
- **§6.5 — so the check compares two declarations, which is what BROKKR already does for classification.** Nothing reads a payload to decide sensitivity: a classification is declared, the gate enforces it deterministically, a heuristic sentinel flags suspected mislabelling without deciding. **Minimization is placed the same way rather than as a second pattern**, per the disposition.
- **§6.5 — the allowed field set lives in the genome, not on `Purpose`, and the reason is the whole security argument.** `Purpose` is `{ description: String }` and travels *on the crossing* inside the BCR. An allowed set carried there would let the party assembling the data declare both what it is admitting **and** what it is permitted to admit — **a producer's claim checked against the same producer's claim**, the circular shape §7 names and the identical defect Rev 1.22 refused when it declined a caller-supplied tier for `promote`: *the thing being governed does not supply the terms of its own governance.* `PurposeFieldPolicy` therefore joins `PolicyRegister`, beside the capability vocabulary and the invariant predicates.
- **§6.5 — reached through a `PurposeFieldResolver` seam, preserving I-5.** `Huth<C: EndpointCeiling, A: AcceptanceResolver>` is already generic over two injected resolvers and `brokkr-barrier` does not depend on `brokkr-genome`; a third preserves that. **An unresolvable purpose refuses rather than permits** — otherwise the control would be removable by declaring a purpose nobody registered, and OQGF-P-2's fail-closed direction is the one §6.2 already chose for an invariant with no declared predicate.
- **§6.5 — the present field set is declared on the crossing and covered by the BCR signature.** `fields` joins `PersonalDataTag`, which already rides on both the flow and the BCR. `bcr_signed_content` → `write_personal` (`canonical.rs:90`) encodes purpose and retention today; extending it makes the declaration tamper-evident. **The cost is stated rather than discovered at implementation: every previously issued BCR carrying a personal tag stops verifying.** That cost is zero today because no deployment issues BCRs, and it would not be zero later.
- **§6.5 — the check refuses rather than quarantines, and that differs from the surrounding ingress rules deliberately.** Quarantine is right for *unprovenanced* data because provenance can be established afterwards and the datum is then admissible unchanged. A datum carrying out-of-scope fields is not waiting on a missing fact — **it is the wrong data for this purpose.** Holding it would imply a later step could make it admissible as it stands. The AMD-006 path past it is unchanged.
- **§6.5 — a seventh error-type correction, checked before assuming.** `BarrierCondition`'s eight committed variants were enumerated; none says a field is outside the declared purpose scope, and the nearest — `PersonalDataUndeclared` — means the **opposite**, that no purpose was declared. Reporting an over-broad admission as an undeclared one would send an operator to add a declaration already present. `PersonalDataFieldOutOfScope` is placed. **The recurrence has one cause, unchanged since Rev 1.27: at the moment of writing the author knows which condition failed, and the reader will not** — and here a vague condition is worse than an unclear message, because `finding_id()` scopes an AMD-006 acceptance to it.
- **§6.5 — the heuristic backstop raises findings and never gates, and the prohibition is structural.** It can flag a declaration that understates structured content it can parse. It cannot reach semantic over-collection *inside* a permitted field — a `notes` field carrying a medical history — which is the covert-channel residual in a new place. **A heuristic that gated would violate OQGF-P-2 twice over:** tolerance never applies to a Deterministic Gate, so a gating inspector would be a fail-closed control a Tolerance Grant could silence — the hole `NonSuppressibleGate` exists to prevent — and the barrier's verdict would stop being deterministic.
- **§6.5, §13 — what it proves is bounded, and stated in the terms of its siblings.** It proves admitted content matches a signed declaration within a DAP-signed field set. **It does not prove the declaration is minimal**: an over-broad policy is enforced faithfully while the requirement's substance goes unmet — **the residual shape of P-12.3's self-declared envelope and R-6.2's self-declared custody model**, governed by AMD-018 §AMD.2.1. What changes is that an over-broad scope becomes a signed, dated, DAP-attributed artifact reviewable in a diff, where minimization is today simply unstated. **The declaration is not the control.**
- **§6.5 — a §5.4 table with two rows recording rules that are not placed.** One is unbuildable and is not designed around; one is not a rule at all. Rev 1.21 placed a predicate against an unreachable tier and Rev 1.24 placed rules against unexposed fields; **both were caught at Step 0 by a builder rather than at drafting**, which is the failure that table exists to prevent.
- **§14 — the verdict does not move, and would move only partly if built.** The declaration half is code; the minimality half is an assessment finding against a live deployment.

**Self-interest labelling per §4.** Two choices reduce my work and should be weighed on their merits rather than on my having made them. **Recording the content-reading rule as unbuildable is the larger one** — it removes the hardest thing that could have been asked for, and I am the party reporting it unreachable. The evidence is falsifiable in one command: `BoundaryFlow::Ingress` carries `DatumRef`, and `ids.rs:41` shows it declared in the `string_id!` block beside `SubjectId` and `ToolId`. **A reader who doubts it should read that block rather than accept this paragraph.** The independent reason stands regardless of reachability: content inspection in a Deterministic Gate is forbidden by OQGF-I-12 whatever the plumbing allows. **The smaller one is recording minimality as assessment rather than designing a predicate for it** — convenient, and also the honest description, since no predicate over the data decides whether a field is *necessary* for a purpose.

**What Rev 1.29 does not do.** It implements nothing — `PersonalDataTag` has no `fields`, `PolicyRegister` has no purpose→field map, `Huth` takes two resolvers not three, and the condition variant does not exist. It moves no verdict. It does not make minimization enforced, and it does not claim that a fully built version would: the half that decides whether a declaration is *minimal* is not reachable by any predicate, and is recorded as an assessor's question. **Drafted for DAP review; not placed, not committed.**
...
— End of BROKKR technical architecture, Rev 1.32.
