...
**Date:** 9 September 2026 (Rev 1.27)
...
|---|---|---|---|
| 1 | **Architecture Revision** | **1.27** | This document. Prior revisions preserved immutably in git (front matter, §15). *(Corrected at Rev 1.27: this row read `1.19` for eight revisions — §1.5 was added at Rev 1.19 and its own first row was never updated. A table whose purpose is to stop a reader confusing the revision with the release state was misstating the revision.)* |
| 2 | **Software State** | **13 crates; 445 tests green; HEAD `9c66bbe`** | `brokkr-{adapt,audit,barrier,bifrost,cli,core,crypto,gate,genome,intent,reasoner,sentinel,tools}` in the workspace. **445 passed / 0 failed / 13 ignored, measured at `9c66bbe` on a quiet tree** (`cargo test --workspace --locked`, sole occupant; the 13 ignored are the live suites, of which the 11 live OQGF-A-3 tests were run separately with `--ignored` and pass). The prior figure, "Workspace GREEN 409/0" from commit `a53cafb`, is superseded. §14 traces the requirement each crate carries. |
...
- **Audit records carry no trusted timestamp; the client that would obtain one is designed and not built.** *(New in Rev 1.9; rewritten at Rev 1.24.)* OQGF-A-3 requires records *"timestamped via an RFC 3161-compliant authority that itself supports PQC signing."* §6.9 defines the seam and records the absence explicitly (`Timestamping::Unavailable`) rather than omitting it, and Rev 1.24 places the client design. **The verdict does not move** — nothing is implemented, and the `brokkr-cli` composition still passes `None` (`brokkr-cli/src/main.rs:363`). **OQGF-A-3 is PARTIAL**: the dual-family signing half is met, the timestamp half is not. What that costs is precise — the ordering of records is BROKKR's own claim rather than a third party's attestation, so the chain proves internal consistency and not independent time. It is **not** reclassified to High-Assurance: A-3 carries no level qualifier in the corpus and binds at Enhanced. Four things remain named rather than closed:
  ...
  - **One error mapping still rests on a header constant rather than observed behaviour.** *(New at Rev 1.26; narrowed at Rev 1.27.)* Rev 1.26 recorded three — `ASN_AFTER_DATE_E`, `ASN_BEFORE_DATE_E`, and `ASN_SIG_CONFIRM_E` — on the belief that an expired certificate could not be produced on the engineering host. **Two are closed:** `openssl ca -startdate/-enddate` issues arbitrary validity windows, and a committed test now drives both date errors. **`ASN_SIG_CONFIRM_E` is not**, and it is worth stating why the count did not simply drop to zero: a source comment asserts all three were observed, but **no committed test drives a certificate to `−155`** — the untrusted-signer test only asserts the error is *not* `SignatureInvalid`, which is a negative assertion. Under §7 the builder's own summary is not evidence, so the row stays open until a test asserts it.
  - **Exclusivity is enforced over recognized purposes only, and an unrecognized OID is invisible to it.** *(New at Rev 1.27.)* The RFC 3161 §2.3 exclusivity clause is placed as `extExtKeyUsage == EXTKEYUSE_TIMESTAMP`, which refuses every extra purpose wolfSSL's decoder recognizes — observed on `timeStamping+clientAuth` (`0x24`) and `timeStamping+anyExtendedKeyUsage` (`0x21`). **It does not refuse an extra purpose the decoder does not recognize.** `DecodeExtKeyUsage` carries an explicit skip-unknown-OIDs branch that sets no bit, and `EXTKEYUSE_USER` is written only by the certificate-generation path, so a leaf carrying `timeStamping` plus a private OID reads `0x20` and passes — observed. The route that would close it, `DecodedCert::extExtKeyUsageCount`, is **absent from this build** (behind `OPENSSL_EXTRA`, verified by compiling), and would not see an unknown purpose even where present. **The bound is shared with the reference implementation**: OpenSSL accepts the same certificate under `-purpose timestampsign`, because it too reduces the EKU to a bitmask of recognized purposes. That makes this the bound standard practice has rather than a weakness peculiar to BROKKR — which is a reason to record it precisely, not a reason to treat it as closed. A CA determined to issue a multi-purpose TSA certificate that passes both implementations can do so with a private OID for the second purpose.
...
— End of BROKKR technical architecture, Rev 1.27.
