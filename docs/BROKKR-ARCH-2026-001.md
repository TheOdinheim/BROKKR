...
| **OQGF-P-8.2, P-8.5 (resolution is an act; no autonomous lowering)** | **`brokkr-core::resolution::ResolutionDecision` with a committed signed-content encoding, a nonce, and an expiry (§6.8, Rev 1.12); EIR refuses a decision that does not verify under the declared DAP key, is expired, or replays a nonce. The encoding lives in core because a decision is signed by one party and verified by another** |
...
— End of BROKKR technical architecture, Rev 1.12.
