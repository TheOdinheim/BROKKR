...
**Date:** 10 September 2026 (Rev 1.29)
...
- **Minimization has a placed design and no implementation; and the design proves conformity to a declaration, never that the declaration is minimal.** *(New in Rev 1.7; rewritten at Rev 1.29.)* OQGF-P-11.2 requires Personal Data admitted to a Privileged Context to be *"minimized to what the declared Purpose requires."* Rev 1.7 gated the *presence* of a declared Purpose and recorded the rest open. Rev 1.29 places the design (§6.5): a signed, DAP-owned `PurposeFieldPolicy` in the genome resolved through a seam, a `fields` declaration on `PersonalDataTag` covered by the BCR signature, a set-containment check, and `BarrierCondition::PersonalDataFieldOutOfScope`. **Nothing is implemented and the verdict does not move.** Three things remain open beneath it, and only the first is closable by code:
  - **The design is not built.** `PersonalDataTag` has no `fields` member, `PolicyRegister` has no purpose→field map, no third resolver is injected into `Huth`, and the condition variant does not exist.
  - **An over-broad declaration is enforced faithfully.** A DAP who signs a policy listing every field the organization holds gets exactly that gated, with the mechanism working as designed and the requirement's substance unmet. **The same residual shape as the self-declared Capability Envelope (P-12.3) and the self-declared custody model (R-6.2)** — AMD-018 §AMD.2.1 governs it: a declaration that overstates what is achieved is a conformance failure, not a documentation defect, and testing it is an assessor's work against a live deployment. What the design changes is that an over-broad scope becomes a signed, dated, DAP-attributed artifact reviewable in a diff, where today minimization is simply unstated.
  - **A declaration that lies is outside the deterministic check entirely.** The gate enforces that a datum matches what it *says* it carries. A tag listing three fields over content carrying five is the content sentinel's business (OQGF-I-12), which raises a finding and **never gates** — a heuristic that gated would be a fail-closed control a Tolerance Grant could silence, violating OQGF-P-2. And semantic over-collection *inside* a declared field — a permitted `notes` field carrying a medical history — is reached by no field-level control at all, which is the covert-channel residual in a new place.
...
| **OQGF-P-11.1, P-11.3, P-11.4 (AMD-009)** | **`brokkr-core::barrier::PersonalDataTag` — orthogonal to `Classification`; carried on both the flow and the BCR; egress condition 9 and the Privileged-Context ingress rule (§6.5, Rev 1.7). PARTIAL for P-11.2 — **Rev 1.29 places the minimization design and the verdict does not move** (§6.5): a signed `PurposeFieldPolicy` in the genome reached through a `PurposeFieldResolver` seam (preserving I-5), a `fields` declaration on `PersonalDataTag` inside the BCR's signed content, set containment as ingress conditions 10 and 11, and `BarrierCondition::PersonalDataFieldOutOfScope`. **Nothing is implemented.** Beneath the verdict Rev 1.29 records what Step 0 established: a rule reading fields *off content* is unbuildable — `BoundaryFlow` carries an opaque `DatumRef` — and would put content inference inside a Deterministic Gate, which OQGF-I-12 forbids; so the check compares two declarations, as classification already does. **Even fully built it would move P-11.2 only partly**: the declaration half is code, the *minimality* half is an assessment finding against a live deployment (§13). P-11.5 crypto-shredding is `brokkr-crypto` (Phase 2); P-11.6/P-11.7 are SAGA (Phase 7)** |
...
— End of BROKKR technical architecture, Rev 1.29.
