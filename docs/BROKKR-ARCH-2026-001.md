...
**Date:** 9 September 2026 (Rev 1.26)
...
| Certificate lacks `id-kp-timeStamping` | *(policy, not a wolfSSL code)* | **observed** | **`NotTimestampingCertificate`** *(new)* | wrong endpoint, or a mis-issued certificate |
...
##### The timestamping EKU — reachable, and placed

RFC 3161 requires the TSA's signing certificate to carry `id-kp-timeStamping` as a **critical** extended key usage. **Nothing checks this today**, in either mode: `wolfSSL_CertManagerVerifyBuffer` performs path validation and does not enforce application EKU policy.

It **is** reachable, by the shim pattern this section already uses. `DecodedCert::extExtKeyUsage` (`asn.h:1852`) is a bitfield and `EXTKEYUSE_TIMESTAMP` is `0x20` (`asn.h:1422`); `wc_InitDecodedCert`, `wc_ParseCert`, and `wc_FreeDecodedCert` are all exported. A shim accessor over that byte completes the check:

```text
wc_InitDecodedCert(dc, leaf_der, n, NULL)
wc_ParseCert(dc, CERT_TYPE, NO_VERIFY, NULL)   -- NO_VERIFY: the chain was already decided
brokkr_cert_ext_key_usage(dc)                  -- new shim accessor over extExtKeyUsage
wc_FreeDecodedCert(dc)
```

`NO_VERIFY` is deliberate and worth stating: **path validation is the CertManager's job and has already happened.** Re-verifying here would be a second, weaker opinion about trust; this parse exists only to read one byte.

**This was observed, not inferred.** Against a purpose-built chain, a leaf carrying `extendedKeyUsage=critical,timeStamping` read `extExtKeyUsage = 0x20`, and its issuing root read `0x00` — so the check distinguishes a TSA leaf from a CA that merely signed one, which is the distinction it exists to make. **The check applies to the leaf only**, as RFC 3161 requires; a root carrying the timestamping EKU would be the anomaly.

**One field-layout note, because it is the same rationale as the PKCS#7 shim.** `extExtKeyUsage` itself is unguarded, but fields *before* it in `DecodedCert` sit behind `WOLFSSL_ASN_CA_ISSUER` and `WOLFSSL_AKID_NAME`. Its offset therefore depends on build flags, so it is read by a shim compiled with the library's own flags rather than by a Rust-side offset — for exactly the reason the accessor shim exists at all.
...
| Map an expired certificate | `ASN_AFTER_DATE_E` (−151) | return code — `brokkr-crypto` | **header only** — see below |
| Map a not-yet-valid certificate | `ASN_BEFORE_DATE_E` (−150) | return code — `brokkr-crypto` | **header only** — see below |
| Map a bad signature | `ASN_SIG_CONFIRM_E` (−155) | return code — `brokkr-crypto` | **header only** |
...
**Three date/signature rows are marked "header only", and that distinction is deliberate.** Their constants were read from `error-crypt.h` with exact values, but **the code paths returning them were not observed** — an expired certificate could not be produced on this host (`faketime` is absent and this OpenSSL's `x509` has no `-not_before`). The mappings are placed because the constants are unambiguous, and they are flagged because **"the constant exists" is the weaker of the two kinds of evidence in this table**, and it is the kind that has been mistaken for the stronger one before. An implementation should close that gap by constructing the certificates a test needs rather than by trusting this row.
...
- **Audit records carry no trusted timestamp; the client that would obtain one is designed and not built.** *(New in Rev 1.9; rewritten at Rev 1.24.)* OQGF-A-3 requires records *"timestamped via an RFC 3161-compliant authority that itself supports PQC signing."* §6.9 defines the seam and records the absence explicitly (`Timestamping::Unavailable`) rather than omitting it, and Rev 1.24 places the client design. **The verdict does not move** — nothing is implemented, and the `brokkr-cli` composition still passes `None` (`brokkr-cli/src/main.rs:363`). **OQGF-A-3 is PARTIAL**: the dual-family signing half is met, the timestamp half is not. What that costs is precise — the ordering of records is BROKKR's own claim rather than a third party's attestation, so the chain proves internal consistency and not independent time. It is **not** reclassified to High-Assurance: A-3 carries no level qualifier in the corpus and binds at Enhanced. Four things remain named rather than closed:
  ...
  - **Three error mappings rest on header constants rather than observed behaviour.** *(New at Rev 1.26.)* `ASN_AFTER_DATE_E`, `ASN_BEFORE_DATE_E`, and `ASN_SIG_CONFIRM_E` were read from `error-crypt.h` with exact values, but the code paths returning them were **not observed** — an expired certificate could not be produced on the engineering host. The distinction is recorded because **"the constant exists" is the weaker kind of evidence, and it is the kind that has been mistaken for the stronger one** in this requirement's history. An implementation closes the gap by constructing the certificates the tests need.
...
— End of BROKKR technical architecture, Rev 1.26.
