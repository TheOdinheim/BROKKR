...
**Date:** 8 September 2026 (Rev 1.24)
...
|---|---|---|
| Request a timestamp over the record | the bytes to imprint | `signed: &[u8]`, the parameter of `Saga::stamp` (`saga.rs:279`), which is `record_signed_content(&record)` (`canonical.rs:698`) |
| Verify signature and chain | the token bytes | the return of `TimestampAuthority::stamp` |
| Verify signature and chain | the TSA trust anchor | **client-held, supplied at construction.** Not reachable from SAGA, and deliberately not routed there — the client is the component that knows which authority it speaks to |
| Verify the imprint binds | the expected digest | the same `signed` slice, in scope inside the client for the duration of the call |
| Record the algorithm | the TSA signature OID | parsed from the token inside the client; carried out on `TimestampToken::algorithm` |
| Classify a failure | which check failed | inside the client, returned as the `TimestampError` variant |
| **Not placed** | *re-verifying a stored token at chain-verification time* | **No path exists.** `Saga::verify_chain` (`saga.rs:~290`) holds no TSA certificate and has no parameter for one. A stored token is verified **once, at stamp time**, and never again by BROKKR |

**The last row is the honest limit and it should not be read past.** RFC 3161 tokens are independently verifiable by anyone holding the TSA certificate — that is the point of the format — so an external assessor *can* re-verify BROKKR's stored tokens. **BROKKR itself does not**, and giving `verify_chain` a trust-anchor parameter is a change this revision does not place, because nothing currently consumes the result and a rule with no consumer is surface without a reader.
...
— End of BROKKR technical architecture, Rev 1.24.
